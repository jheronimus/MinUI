...
static void keyboard_drawCenteredText(TTF_Font* face, const char* text, SDL_Color color, SDL_Surface* screen,
...
void UI_KEYBOARD_draw(const struct ui_keyboard* keyboard, SDL_Surface* screen, const SDL_Rect* rect) {
	...
	for (row = 0; row < UI_KEYBOARD_ROWS; row++) {
		...
		for (col = 0; col < row_def->count; col++) {
			...
			keyboard_drawCenteredText(face, key, selected ? COLOR_BLACK : COLOR_WHITE, screen, x, y, width,
			                          row_height);
