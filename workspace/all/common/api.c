...
uint32_t asset_rgbs[ASSET_COLORS + 1] = {
    [ASSET_WHITE_PILL] = 0xffffff,
    [ASSET_BLACK_PILL] = 0x000000,
    [ASSET_DARK_GRAY_PILL] = 0x363636,
    [ASSET_OPTION] = 0x404040,
    [ASSET_BUTTON] = 0x404040,
    [ASSET_PAGE_BG] = 0xaaaaaa,
    [ASSET_STATE_BG] = 0x858585,
    [ASSET_PAGE] = 0xffffff,
    [ASSET_BAR] = 0xffffff,
    [ASSET_BAR_BG] = 0x939393,
    [ASSET_BAR_BG_MENU] = 0x939393,
    [ASSET_UNDERLINE] = 0x494949,
    [ASSET_DOT] = 0x8c8c8c,
    [ASSET_HOLE] = 0x6c6c6c,
};
...
SDL_Surface *GFX_init(int mode) {
    ...
    // init image loading
    int mask = IMG_INIT_PNG;
    if (!(IMG_Init(mask) & mask))
        LOG_info("IMG_Init: %s\n", IMG_GetError());
    ...
    // load assets
    gfx.assets = PLAT_initVideo();
    if (!gfx.assets) {
        LOG_info("GFX_init: no assets\n");
        return NULL;
    }

    // set colors
    SDL_Surface *white = SDL_CreateRGBSurface(0, 1, 1, FIXED_DEPTH, RGBA_MASK_AUTO);
    SDL_FillRect(white, NULL, SDL_MapRGBA(white->format, TRIAD_WHITE, 255));
    RGB_WHITE = ((uint32_t *)white->pixels)[0];
    SDL_Surface *black = SDL_CreateRGBSurface(0, 1, 1, FIXED_DEPTH, RGBA_MASK_AUTO);
    SDL_FillRect(black, NULL, SDL_MapRGBA(black->format, TRIAD_BLACK, 255));
    RGB_BLACK = ((uint32_t *)black->pixels)[0];
    SDL_Surface *light_gray = SDL_CreateRGBSurface(0, 1, 1, FIXED_DEPTH, RGBA_MASK_AUTO);
    SDL_FillRect(light_gray, NULL, SDL_MapRGBA(light_gray->format, TRIAD_LIGHT_GRAY, 255));
    RGB_LIGHT_GRAY = ((uint32_t *)light_gray->pixels)[0];
    SDL_Surface *gray = SDL_CreateRGBSurface(0, 1, 1, FIXED_DEPTH, RGBA_MASK_AUTO);
    SDL_FillRect(gray, NULL, SDL_MapRGBA(gray->format, TRIAD_GRAY, 255));
    RGB_GRAY = ((uint32_t *)gray->pixels)[0];
    SDL_Surface *dark_gray = SDL_CreateRGBSurface(0, 1, 1, FIXED_DEPTH, RGBA_MASK_AUTO);
    SDL_FillRect(dark_gray, NULL, SDL_MapRGBA(dark_gray->format, TRIAD_DARK_GRAY, 255));
    RGB_DARK_GRAY = ((uint32_t *)dark_gray->pixels)[0];

    return gfx.screen = GFX_resize(FIXED_WIDTH, FIXED_HEIGHT, FIXED_PITCH);
}
...
// TODO: only supports GPU copy
void GFX_flip(SDL_Surface *screen) {
    if (gfx.vsync)
        PLAT_vsync(gfx.vsync);
    PLAT_flip(screen, gfx.vsync);
}
void GFX_sync(void) {
    if (gfx.vsync)
        PLAT_vsync(gfx.vsync);
...
static int blend16_mono(uint32_t c1, uint32_t c2) {
    int r = ((c1 & 0xf800) + (c2 & 0xf800)) >> 1;
    int g = ((c1 & 0x07e0) + (c2 & 0x07e0)) >> 1;
    int b = ((c1 & 0x001f) + (c2 & 0x001f)) >> 1;
    r &= 0xf800;
    g &= 0x07e0;
    b &= 0x001f;
    return r | g | b;
}
static int blend32_mono(uint32_t c1, uint32_t c2) {
    uint32_t a = ((c1 & 0xff000000) + (c2 & 0xff000000)) >> 1;
    uint32_t r = ((c1 & 0x00ff0000) + (c2 & 0x00ff0000)) >> 1;
    uint32_t g = ((c1 & 0x0000ff00) + (c2 & 0x0000ff00)) >> 1;
    uint32_t b = ((c1 & 0x000000ff) + (c2 & 0x000000ff)) >> 1;
    a &= 0xff000000;
    r &= 0x00ff0000;
    g &= 0x0000ff00;
    b &= 0x000000ff;
...
static int blend16(uint32_t c1, uint32_t c2) {
    switch (FIXED_DEPTH) {
    case 16:
        return blend16_mono(c1, c2);
    case 32:
        return blend32_mono(c1, c2);
    default:
        return 0;
    }
}
...
static void scaleAA(void *__restrict src, void *__restrict dst, uint32_t w, uint32_t h, uint32_t pitch, uint32_t dst_w,
                    uint32_t dst_h, uint32_t dst_p) {
    int dy = 0;
    int lines = h;
...
scaler_t GFX_getAAScaler(GFX_Renderer *renderer) {
    ...
    double blend_denominator =
        (renderer->src_w > renderer->dst_w) ? 5 : 2.5; // TODO: these values are really only good for the nano...
...
void GFX_blitButton(char *hint, char *button, SDL_Surface *dst, SDL_Rect *dst_rect) {
    ...
    if (strlen(button) == 1) {
        ...
        GFX_blitPill(ASSET_BUTTON, dst,
                     &(SDL_Rect){dst_rect->x, dst_rect->y, SCALE1(BUTTON_SIZE) / 2 + text->w, SCALE1(BUTTON_SIZE)});
        ox += SCALE1(BUTTON_SIZE) / 4;

        int oy = special_case ? SCALE1(-2) : 0;
        SDL_BlitSurface(
            text, NULL, dst,
            &(SDL_Rect){ox + dst_rect->x, oy + dst_rect->y + (SCALE1(BUTTON_SIZE) - text->h) / 2, text->w, text->h});
        ox += text->w;
        ox += SCALE1(BUTTON_SIZE) / 4;
        SDL_FreeSurface(text);
    }
    ...
    SDL_BlitSurface(text, NULL, dst,
                    &(SDL_Rect){ox + dst_rect->x, dst_rect->y + (SCALE1(BUTTON_SIZE) - text->h) / 2, text->w, text->h});
    SDL_FreeSurface(text);
}
...
static void PWR_runSleepTransition(int *dirty, uint32_t *last_input_at, uint32_t *power_pressed_at,
...
static void PWR_applyTriggerBehavior(int behavior, int *dirty, uint32_t *last_input_at, uint32_t *power_pressed_at,
                                     PWR_callback_t before_sleep, PWR_callback_t after_sleep) {
    switch (behavior) {
    ...
    default:
        if (!pwr.can_sleep)
            return;
        PWR_runSleepTransition(dirty, last_input_at, power_pressed_at, before_sleep, after_sleep);
        break;
    }
}
...
#define CHARGE_DELAY 1000
    ...
    if (PAD_justReleased(BTN_POWEROFF) ||
        (power_pressed_at && PAD_isPressed(BTN_POWER) && now - power_pressed_at >= 1000)) {
        power_pressed_at = 0;
        if (before_sleep)
            before_sleep();
        PWR_powerOff();
    ...
    if (pwr.sleep_timeout_ms > 0 && now - last_input_at >= pwr.sleep_timeout_ms) {
    ...
    if (pwr.requested_lid_action) {
        pwr.requested_lid_action = 0;
        PWR_applyTriggerBehavior(pwr.lid_behavior, &dirty, &last_input_at, &power_pressed_at, before_sleep,
                                 after_sleep);
        now = SDL_GetTicks();
    } else if (short_power_release) {
        PWR_applyTriggerBehavior(pwr.power_button_behavior, &dirty, &last_input_at, &power_pressed_at, before_sleep,
                                 after_sleep);
        now = SDL_GetTicks();
    ...
    // TODO: only delay hiding setting changes if that setting didn't require a modifier button be held, otherwise
    // release as soon as modifier is released
...
static void PWR_enterSleep(void) {
    ...
    system("killall -STOP keymon");

    sync();
}
static void PWR_exitSleep(void) {
    system("killall -CONT keymon");
    if (GetHDMI()) {
        // buh
    } else {
