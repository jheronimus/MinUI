...
// Compiled inline from minarch.c (see `#include "rewind.c"`), so all of
// minarch.c's declarations (core, config, input_poll_callback, setFastForward,
// LOG_*) are in scope here.
...
static int Rewind_entryOverlapsRangeLocked(int entry_index, size_t start, size_t end) {
...
static int Rewind_writeEntryLocked(const uint8_t *src, size_t src_size, size_t state_size, int is_keyframe) {
    size_t write_offset;
    ...
    while (rewind_ctx.entry_count) {
        ...
        if (!Rewind_entryOverlapsRangeLocked(oldest, write_offset, write_offset + src_size)) {
            break;
        }
        Rewind_dropOldestLocked();
    }

    while (rewind_ctx.entry_count && Rewind_freeSpaceLocked() <= src_size) {
        Rewind_dropOldestLocked();
    }

    if (rewind_ctx.entry_count && Rewind_freeSpaceLocked() <= src_size) {
        return 0;
    }
...
static int Rewind_compressStateLocked(const uint8_t *src, size_t state_size, int force_keyframe, size_t *dest_size,
                                      int *is_keyframe) {
    ...
    if (!keyframe && (!rewind_ctx.has_prev_enc || rewind_ctx.prev_state_size != state_size)) {
        keyframe = 1;
    }

    if (!keyframe) {
        for (i = 0; i < state_size; i++) {
            rewind_ctx.delta_buf[i] = src[i] ^ rewind_ctx.prev_state_enc[i];
        }
        compress_src = rewind_ctx.delta_buf;
    }

    result = LZ4_compress_fast((const char *)compress_src, (char *)rewind_ctx.scratch, (int)state_size,
...
static int Rewind_captureState(uint8_t *state, size_t state_size) {
    ...
    rewind_ctx.savestate_context = RETRO_SAVESTATE_CONTEXT_RUNAHEAD_SAME_INSTANCE;
...
static int Rewind_captureSync(int force_keyframe) {
    ...
    pthread_mutex_lock(&rewind_ctx.lock);
    if (!Rewind_compressStateLocked(rewind_ctx.state_buf, state_size, force_keyframe, &dest_size, &is_keyframe)) {
        pthread_mutex_unlock(&rewind_ctx.lock);
        return 0;
    }
    if (!Rewind_writeEntryLocked(rewind_ctx.scratch, dest_size, state_size, is_keyframe)) {
        pthread_mutex_unlock(&rewind_ctx.lock);
        return 0;
    }
    pthread_mutex_unlock(&rewind_ctx.lock);
...
static int Rewind_shouldCapture(uint32_t now_ms) {
    if (!rewind_ctx.last_capture_ms)
        return 1;
    return (int)(now_ms - rewind_ctx.last_capture_ms) >= rewind_ctx.capture_interval_ms;
}

static int Rewind_shouldKeyframe(uint32_t now_ms) {
    if (!rewind_ctx.last_keyframe_ms)
        return 1;
    return (int)(now_ms - rewind_ctx.last_keyframe_ms) >= rewind_ctx.keyframe_interval_ms;
}

static int Rewind_queueCapture(size_t state_size, int force_keyframe, uint32_t now_ms) {
    int slot = -1;
...
static int Rewind_buildStateLocked(int target_index, size_t *state_size_out) {
    ...
    for (i = keyframe_index; i <= target_index; i++) {
        ...
        result = LZ4_decompress_safe((const char *)rewind_ctx.buffer + entry->offset,
                                     (char *)(entry->is_keyframe ? rewind_ctx.state_buf : rewind_ctx.delta_buf),
        ...
        if (!entry->is_keyframe) {
            for (j = 0; j < state_size; j++) {
                rewind_ctx.state_buf[j] ^= rewind_ctx.delta_buf[j];
            }
        }
    }
...
static void Rewind_trimPlayback(void) {
    ...
    rewind_ctx.entry_head = (rewind_ctx.entry_tail + keep_count) % rewind_ctx.entry_capacity;
    rewind_ctx.head = Rewind_entryEnd(entry);
    memcpy(rewind_ctx.prev_state_enc, rewind_ctx.state_buf, rewind_ctx.play_state_size);
...
static int Rewind_initInternal(size_t state_size) {
    ...
    if (!rewind_ctx.buffer || !rewind_ctx.state_buf || !rewind_ctx.delta_buf || !rewind_ctx.scratch ||
    ...
    if (!rewind_ctx.capture_pool || !rewind_ctx.capture_sizes || !rewind_ctx.capture_keyframe ||
        !rewind_ctx.capture_gen || !rewind_ctx.capture_busy || !rewind_ctx.free_stack || !rewind_ctx.queue) {
        Rewind_free();
        return 0;
    }
    ...
    if (pthread_create(&rewind_ctx.worker, NULL, Rewind_workerThread, NULL) == 0) {
    ...
    LOG_info("Rewind: enabled state=%zu alloc=%zu buffer=%iMB capture=%ims "
             ...
             rewind_ctx.state_size, rewind_ctx.alloc_size, rewind_ctx.buffer_mb, rewind_ctx.capture_interval_ms,
             rewind_ctx.keyframe_interval_ms, rewind_ctx.lz4_acceleration,
...
static void *Rewind_workerThread(void *arg) {
    ...
    while (1) {
        ...
        if (generation == current_generation) {
            pthread_mutex_lock(&rewind_ctx.lock);
            if (Rewind_compressStateLocked(rewind_ctx.capture_pool[slot], state_size, rewind_ctx.capture_keyframe[slot],
                                           &dest_size, &is_keyframe)) {
                Rewind_writeEntryLocked(rewind_ctx.scratch, dest_size, state_size, is_keyframe);
            }
            pthread_mutex_unlock(&rewind_ctx.lock);
        }
...
int Rewind_processFrame(void) {
    ...
    now_ms = SDL_GetTicks();
    if (rewind_ctx.last_step_ms && (int)(now_ms - rewind_ctx.last_step_ms) < rewind_ctx.capture_interval_ms) {
        return 1;
    }
    ...
    target_index = rewinding ? (rewind_ctx.play_index - 1) : (rewind_ctx.entry_count - 2);
    if (target_index < 0) {
        pthread_mutex_unlock(&rewind_ctx.lock);
        if (rewind_toggle)
            rewind_toggle = 0;
