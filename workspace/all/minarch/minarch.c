...
static int rewind_cfg_lz4_acceleration = 2;
static int rewind_init_ready = 0;
  ...
  if (suffixMatch(".zip", game.path)) {
    ...
    if (!supports_zip) {
      ...
      while (1) {
        ...
        // TODO: we need to clear game.tmp_path if anything below this point
        // fails!
...
static void State_read(void) { // from picoarch
  ...
  if (!state_file) {
    if (state_slot !=
        8) { // st8 is a default state in MiniUI and may not exist, that's okay
...
static char *rewind_granularity_labels[] = {
    "16 ms (~60 fps)",  "22 ms (~45 fps)",
    "25 ms (~40 fps)",  "33 ms (~30 fps)",
    "50 ms (~20 fps)",  "66 ms (~15 fps)",
    "100 ms (~10 fps)", "150 ms (~7 fps)",
    "200 ms (~5 fps)",  NULL,
};
...
static char *rewind_compression_accel_labels[] = {
    "1 (best ratio)", "2 (default)",  "4 (fast)",
    "8 (faster)",     "12 (fastest)", NULL,
};
...
static char *button_labels_menu[] = {
    ...
    "R2",        "L3",         "R3",      "MENU+UP",    "MENU+DOWN",
    "MENU+LEFT", "MENU+RIGHT", "MENU+A",  "MENU+B",     "MENU+C",
    "MENU+X",    "MENU+Y",     "MENU+Z",  "MENU+START", "MENU+SELECT",
                 ...
                 [FE_OPT_REWIND_BUFFER] =
                     {
                         ...
                         .desc = "Memory reserved for rewind "
                                 "snapshots.\nIncrease for longer rewind "
                                 "times.",
                         .default_value = 2, // 64MB
                         .value = 2,         // 64MB
                         .count = 7,
                 ...
                 [FE_OPT_REWIND_GRANULARITY] =
                     {
                         ...
                         .desc = "Interval between rewind snapshots.\nShorter "
                                 "intervals improve smoothness during\nrewind, "
                 ...
                 [FE_OPT_REWIND_COMPRESSION] =
                     {
                         ...
                         .desc = "Compress rewind snapshots to save\nmemory at "
                                 "the cost of CPU.",
                         .default_value = 1,
                         .value = 1,
                         .count = 2,
                 ...
                 [FE_OPT_REWIND_COMPRESSION_ACCEL] =
                     {
                         ...
                         .desc = "LZ4 acceleration used for rewind\nsnapshots. "
...
static void Config_syncFrontend(char *key, int value) {
  ...
  if (exactMatch(key, config.frontend.options[FE_OPT_SCALING].key)) {
    ...
    rewind_cfg_enable = value;
    i = FE_OPT_REWIND_ENABLE;
  } else if (exactMatch(key,
                        config.frontend.options[FE_OPT_REWIND_BUFFER].key)) {
    rewind_cfg_buffer_mb = strtol(rewind_buffer_labels[value], NULL, 10);
    i = FE_OPT_REWIND_BUFFER;
  } else if (exactMatch(
                 key, config.frontend.options[FE_OPT_REWIND_GRANULARITY].key)) {
    rewind_cfg_granularity = strtol(rewind_granularity_values[value], NULL, 10);
    i = FE_OPT_REWIND_GRANULARITY;
  } else if (exactMatch(key,
                        config.frontend.options[FE_OPT_REWIND_AUDIO].key)) {
    rewind_cfg_audio = value;
    i = FE_OPT_REWIND_AUDIO;
  } else if (exactMatch(
                 key, config.frontend.options[FE_OPT_REWIND_COMPRESSION].key)) {
    rewind_cfg_compress = value;
    i = FE_OPT_REWIND_COMPRESSION;
  } else if (exactMatch(key,
                        config.frontend.options[FE_OPT_REWIND_COMPRESSION_ACCEL]
                            .key)) {
    rewind_cfg_lz4_acceleration =
        strtol(rewind_compression_accel_values[value], NULL, 10);
  ...
  option->value = value;
  // defer Rewind_init until after the explicit init in main(), otherwise cores
  // crash
...
static void Config_getPath(char *filename, int override) {
  if (override)
    sprintf(filename, "%s/%s.cfg", core.config_dir, game.name);
  else
...
static void input_poll_callback(void) {
  ...
  if (PAD_justPressed(BTN_POWER)) {
    ...
    if (!thread_video && was_threaded) {
      // LOG_info("released power with previously threaded core before power
      // off...\n");
      was_threaded = 0;
      toggle_thread = 1;
    }
    ...
    if (!mapping->mod || PAD_isPressed(mod_btn)) {
      if (i == SHORTCUT_TOGGLE_FF) {
        ...
        case SHORTCUT_CYCLE_EFFECT:
          ...
          Config_syncFrontend(config.frontend.options[FE_OPT_EFFECT].key,
                              screen_effect);
          break;
        default:
          break;
...
static bool environment_callback(unsigned cmd,
  ...
  // requests frontend to handle rotation 	break;
  // }
  ...
  // TODO: I think this is where the core reports its variables (the precursor
  // to options)
  // TODO: this is called if RETRO_ENVIRONMENT_GET_CORE_OPTIONS_VERSION sets out
  // to 0
  ...
  case RETRO_ENVIRONMENT_SET_SERIALIZATION_QUIRKS: { /* consumed; not used by
                                                        rewind buffer */
    break;
  }
  case RETRO_ENVIRONMENT_SET_DMG_GRID_COLOR: { /* private, from the gambatte
                                                  core patch */
  ...
  // RETRO_ENVIRONMENT_EXPERIMENTAL) RETRO_ENVIRONMENT_GET_VFS_INTERFACE (45 |
  // RETRO_ENVIRONMENT_EXPERIMENTAL) RETRO_ENVIRONMENT_GET_AUDIO_VIDEO_ENABLE
  // (47 | RETRO_ENVIRONMENT_EXPERIMENTAL) RETRO_ENVIRONMENT_GET_INPUT_BITMASKS
  ...
  case RETRO_ENVIRONMENT_SET_CORE_OPTIONS_DISPLAY: { /* 55 */
    ...
    // 	if (display) OptionList_setOptionVisibility(&config.core, display->key,
    // display->visible);
    break;
  }
  ...
  case RETRO_ENVIRONMENT_SET_CONTENT_INFO_OVERRIDE: { /* 65 */
    // const struct retro_system_content_info_override* info = (const struct
    // retro_system_content_info_override* )data; if (info) LOG_info("has
    // overrides");
    break;
  }
  ...
  // used by fceumm
  // TODO: used by gambatte for L/R palette switching (seems like it needs to
  // return true even if data is NULL to indicate support)
...
static void buffer_downsample(const void *data, unsigned width, unsigned height,
  ...
  for (int y = 0; y < height; y++) {
    ...
    input += extra; // TODO: commenting this out fixes geolith when cropped, it
                    // appears to be lying about the pitch...
  }
}
...
static void selectScaler(int src_w, int src_h, int src_p) {
  ...
  if (scaling == SCALE_NATIVE || scaling == SCALE_CROPPED) {
    ...
    // TODO: or is it just that I'm trying to cram 4 logical rects into 2 rect
    // arguments
    ...
    // odd resolutions (eg. PS1 Rayman: 320x239) is throwing this off, need to
    // snap to eights
...
static void video_refresh_callback_main(const void *data, unsigned width,
  ...
  // at 60fps 14 will let GB hit 10x but NES and SNES will drop to 1.5x at 30fps
  // (not sure why) but 10 hurts PS...
  // TODO: 10 was based on rg35xx, probably different results on other supported
  // platforms
...
void Core_open(const char *core_path, const char *tag_name) {
  ...
  LOG_info(
      "core: %s version: %s tag: %s (valid_extensions: %s need_fullpath: %i)\n",
...
void Core_load(void) {
  ...
  core.set_controller_port_device(
      0,
      RETRO_DEVICE_JOYPAD); // set a default, may update after loading configs
...
void Menu_init(void) {
  menu.overlay = SDL_CreateRGBSurface(
      SDL_SWSURFACE, DEVICE_WIDTH, DEVICE_HEIGHT, FIXED_DEPTH, RGBA_MASK_AUTO);
...
static MenuList OptionEmulator_menu = {
    .type = MENU_FIXED,
    .on_confirm = OptionEmulator_optionDetail, // TODO: this needs pagination to
                                               // be truly useful
...
static void Menu_scale(SDL_Surface *src, SDL_Surface *dst) {
  ...
  if (scaling == SCALE_ASPECT || rw > dw || rh > dh) {
    ...
    if (core_aspect > fixed_aspect) {
      ...
      rw += rw % 2;
      rw = (rw / 8) *
           8; // probably necessary here since we're not scaling by an integer
    } else {
...
static void Menu_loop(void) {
  ...
  while (show_menu) {
    ...
    if (dirty) {
      ...
      int text_width = GFX_truncateText(font.large, rom_name, display_name,
      ...
      GFX_blitPill(ASSET_BLACK_PILL, screen,
                   &(SDL_Rect){SCALE1(PADDING), SCALE1(PADDING), max_width,
                               SCALE1(PILL_SIZE)});
      SDL_BlitSurface(
          text,
      ...
      if (show_setting && !GetHDMI())
        ...
        GFX_blitButtonGroup(
            (char *[]){BTN_SLEEP == BTN_POWER ? "POWER" : "MENU", "SLEEP",
                       NULL},
            0, screen, 0);
      GFX_blitButtonGroup((char *[]){"B", "BACK", "A", "OKAY", NULL}, 1, screen,
                          1);
...
#define PAGINATION_HEIGHT 6
        ...
        int ph =
            hh + SCALE1(WINDOW_RADIUS * 2 + PAGINATION_HEIGHT + WINDOW_RADIUS);
  ...
  if (!quit) {
    ...
    PWR_powerOff(); // TODO: won't work with threaded core, only check this once
                    // per launch
  ...
  if (fast_forward && max_ff_speed) {
    ...
    if (elapsed > 0 && elapsed < 0x80000) {
      if (elapsed < ff_frame_time) {
        ...
        if (delay > 0 &&
            delay < 17) { // don't allow a delay any greater than a frame
          SDL_Delay(delay);
        }
      }
...
static int Rewind_buffer_state_locked(void) {
...
static size_t Rewind_free_space_locked(void) {
  int state = Rewind_buffer_state_locked();
  if (state == 2)
    return 0;
  if (state == 0)
...
static int Rewind_entry_overlaps_range(int entry_idx, size_t range_start,
                                       size_t range_end) {
...
static int Rewind_write_entry_locked(const uint8_t *compressed, size_t dest_len,
  ...
  while (rewind_ctx.entry_count > 0) {
    ...
    if (Rewind_entry_overlaps_range(oldest_idx, write_offset,
                                    write_offset + dest_len)) {
      Rewind_drop_oldest_locked();
    } else {
      break;
    }
  ...
  if (rewind_ctx.entry_count > 0 && Rewind_free_space_locked() <= dest_len) {
    LOG_error("Rewind: unable to make room for entry\n");
    return 0;
  }
...
static int Rewind_compress_state(const uint8_t *src, size_t *dest_len,
  ...
  int res =
      LZ4_compress_fast((const char *)compress_src, (char *)rewind_ctx.scratch,
                        (int)rewind_ctx.state_size, max_dst, accel);
  if (res <= 0)
    return -1;
  *dest_len = (size_t)res;
  if (is_keyframe_out)
...
static void Rewind_worker_thread_inner(int slot) {
  size_t dest_len = rewind_ctx.scratch_size;
  int is_keyframe = 1;

  pthread_mutex_lock(&rewind_ctx.lock);
  if (rewind_ctx.capture_gen[slot] == rewind_ctx.generation) {
...
static void *Rewind_worker_thread(void *arg) {
  ...
  while (1) {
    ...
    Rewind_worker_thread_inner(slot);
...
static void Rewind_init(void) {
  ...
  if (state_size > 4 * 1024 * 1024) {
    LOG_info("Rewind: state size (%zu bytes) exceeds 4MB real-time limit, "
             "disabling\n",
             state_size);
    return;
  }
  ...
  if (!rewind_ctx.compress && rewind_ctx.capacity <= state_size) {
    LOG_warn("Rewind: raw snapshots (%zu bytes) do not fit in buffer; falling "
             "back to compression\n",
             state_size);
  ...
  LOG_info("Rewind: enable=%i bufferMB=%i interval=%ims audio=%i "
           "compression=%s (accel=%i)\n",
  ...
  if (!rewind_ctx.prev_state_enc || !rewind_ctx.prev_state_dec ||
      !rewind_ctx.delta_buf) {
  ...
  LOG_info("Rewind: capture_ms=%d, playback_ms=%d (state=%zu bytes, buffer=%zu "
           "bytes, entries=%d)\n",
           capture_ms, playback_ms, state_size, rewind_ctx.capacity,
           rewind_ctx.entry_capacity);
  ...
  rewind_ctx.pool_size = (state_size > REWIND_LARGE_STATE_THRESHOLD)
                             ? REWIND_POOL_SIZE_LARGE
                             : REWIND_POOL_SIZE_SMALL;
  ...
  if (pthread_create(&rewind_ctx.worker, NULL, Rewind_worker_thread, NULL) !=
      0) {
    LOG_error("Rewind: failed to start worker thread, falling back to "
              "synchronous capture\n");
  ...
  LOG_info("Rewind: enabled (%zu bytes buffer, cadence %i ms)\n",
...
static void Rewind_push(int force) {
  ...
  if (rewind_ctx.worker_running && rewind_ctx.pool_size) {
    ...
    while (1) {
      ...
      if (rewind_ctx.queue_count > 0) {
        ...
        if (gen == rewind_ctx.generation)
          Rewind_worker_thread_inner(queued_slot);
    ...
    return;
  {
    ...
    pthread_mutex_lock(&rewind_ctx.lock);
    int res =
        Rewind_compress_state(rewind_ctx.state_buf, &dest_len, &is_keyframe);
    if (res != 0) {
      pthread_mutex_unlock(&rewind_ctx.lock);
      return;
    }
...
static int Rewind_step_back(void) {
  ...
  if (rewind_ctx.playback_interval_ms > 0 && rewind_ctx.last_step_ms &&
      (int)(now_ms - rewind_ctx.last_step_ms) <
          rewind_ctx.playback_interval_ms) {
  ...
  if (rewind_ctx.compress) {
    int res = LZ4_decompress_safe((const char *)rewind_ctx.buffer + e->offset,
                                  (char *)rewind_ctx.delta_buf, (int)e->size,
                                  (int)rewind_ctx.state_size);
    if (res < (int)rewind_ctx.state_size) {
      LOG_error("Rewind: decompress failed (res=%i, want=%zu)\n", res,
                rewind_ctx.state_size);
      decode_ok = 0;
      ...
      LOG_warn("Rewind: delta frame without previous state, results may be "
               "incorrect\n");
      memcpy(rewind_ctx.state_buf, rewind_ctx.delta_buf, rewind_ctx.state_size);
      if (rewind_ctx.prev_state_dec) {
        memcpy(rewind_ctx.prev_state_dec, rewind_ctx.state_buf,
    ...
    if (e->size != rewind_ctx.state_size) {
      LOG_error("Rewind: raw snapshot size mismatch (got=%zu, want=%zu)\n",
                e->size, rewind_ctx.state_size);
      decode_ok = 0;
    } else {
      memcpy(rewind_ctx.state_buf, rewind_ctx.buffer + e->offset,
             rewind_ctx.state_size);
    }
  }
  if (!decode_ok) {
  ...
  rewind_savestate_context = RETRO_SAVESTATE_CONTEXT_RUNAHEAD_SAME_INSTANCE;
  if (!core.unserialize(rewind_ctx.state_buf, rewind_ctx.state_size)) {
    rewind_savestate_context = RETRO_SAVESTATE_CONTEXT_NORMAL;
...
static void Rewind_sync_encode_state(void) {
  ...
  pthread_mutex_lock(&rewind_ctx.lock);
  if (rewind_ctx.has_prev_dec && rewind_ctx.prev_state_dec &&
...
static void run_frame(void) {
  ...
  // pauses rewind
  int do_rewind =
      (rewind_pressed || rewind_toggle) && !(rewind_toggle && ff_hold_active);
  if (do_rewind) {
...
int main(int argc, char *argv[]) {
  ...
  menu_screen = screen;
  menu_before_sleep = Menu_beforeSleep;
  menu_after_sleep = Menu_afterSleep;
  menu_hdmi_monitor = hdmimon;
  menu_button_labels = button_labels;
  menu_update_desc = OptionSaveChanges_updateDesc;
  ...
  Game_open(
      rom_path); // nes tries to load gamegenie setting before this returns ffs
  if (!game.is_open)
    goto finish;
