...
// EVIOCGNAME from <linux/input.h> without pulling in conflicting BTN_* macros
#ifndef EVIOCGNAME
...
static int MINIME_inputOpenByName(const char *expected) {
    char name[256];
    char path[64];
...
static int MINIME_inputOpenShortcutDevices(int *fds, size_t max_fds) {
    const char *names[3];
    int count = 0;
...
static int MINIME_inputHasCZ(void) {
    return traits && traits->key_c >= 0 && traits->key_z >= 0;
}

static int MINIME_inputNormalizeAxis(int value, int invert) {
    int normalized;
...
static int readCharging(int *charging) {
    char path[MINIME_TRAIT_PATH_MAX + 32];
    char status[32];
    FILE *file;
...
static int MINIME_powerGetBattery(int *charging, int *capacity) {
        ...
        while (read(input, &event, sizeof(event)) == sizeof(event)) {
            ...
            if (type == EV_KEY) {
                ...
                if (code == k_up) {
                    ...
                    btn = BTN_R3;
                    id = BTN_ID_R3;
                } else if (code == k_vol_up) {
                    btn = BTN_PLUS;
                    id = BTN_ID_PLUS;
                } else if (code == k_vol_down) {
                    btn = BTN_MINUS;
                    id = BTN_ID_MINUS;
