...
static void storedPathToAbsolute(const char *stored, char *out, size_t out_size) {
    if (!stored || !stored[0]) {
        out[0] = '\0';
        return;
    }
    if (prefixMatch((char *)SDCARD_PATH, (char *)stored)) {
        snprintf(out, out_size, "%s", stored);
        return;
    }
    snprintf(out, out_size, "%s%s", SDCARD_PATH, stored);
}

static void resolveSaveIdForPath(const char *rom_path, char *out_save_id, size_t out_save_id_len) {
    char core_registry_name[256];
    if (!out_save_id || out_save_id_len == 0)
        return;
    out_save_id[0] = '\0';
    if (!rom_path || !rom_path[0])
        return;

    getEmuName(rom_path, core_registry_name);
    if (CORE_REGISTRY_resolveSaveIdForSystem(core_registry_name, out_save_id, out_save_id_len) == 0)
        return;
...
static int isSafeVersionName(const char *name) {
    return name && name[0] && !strchr(name, '/') && !strchr(name, '\\') && strcmp(name, VERSION_MARKER);
}
...
static int writeVersionChoice(const char *folder, const char *path) {
    ...
    if (!name || !isSafeVersionName(++name))
        return 0;
...
typedef struct Entry {
    ...
    char *unique;
    int type;
    int alpha; // index in parent Directory's alphas Array, which points to the index of an Entry in its entries Array
               // :sweat_smile:
} Entry;
...
static void getUniqueName(Entry *entry, char *out_name) {
    ...
    char core_registry_tag[256];
    getEmuName(entry->path, core_registry_tag);
    ...
    if (exists(map_path)) {
        ...
        if (file) {
            ...
            if (filter) {
                ...
                Array_free(self->entries); // not EntryArray_free because we've just moved the entries from the original
                                           // to the filtered one!
...
static int hasEmu(char *core_registry_name);
static Recent *Recent_new(char *path, char *alias) {
    ...
    char core_registry_name[256];
    getEmuName(sd_path, core_registry_name);

    self->path = strdup(path);
    self->alias = alias ? strdup(alias) : NULL;
    self->available = hasEmu(core_registry_name);
    return self;
}
...
static int hasEmu(char *core_registry_name) {
    char core_path[256];
    getEmuPath(core_registry_name, core_path);
    return exists(core_path);
}
...
static int hasRecents(void) {
    ...
    FILE *file = stat(RECENT_PATH, &recent_stat) == 0 && recent_stat.st_size > 0 ? fopen(RECENT_PATH, "r")
                                                                                 : NULL; // newest at top
    if (file) {
        char line[256];
        while (fgets(line, 256, file) != NULL) {
...
static int hasRoms(char *dir_name) {
    ...
    getEmuName(dir_name, core_registry_name);

    // check for an installed core mapping
    if (!hasEmu(core_registry_name))
        return has;
...
static Array *getRecents(void) {
    ...
    for (int i = 0; i < recents->count; i++) {
        ...
        Entry *entry = Entry_new(sd_path, isDirPath(sd_path) ? ENTRY_DIR : ENTRY_ROM);
        if (recent->alias) {
            free(entry->name);
...
static Array *getCollection(char *path) {
    ...
    if (file) {
        ...
        while (fgets(line, 256, file) != NULL) {
            ...
            if (exists(sd_path)) {
                Array_push(entries, Entry_new(sd_path, isDirPath(sd_path) ? ENTRY_DIR : ENTRY_ROM));
            }
        }
        fclose(file);
    }
...
static int resolveDirectoryLaunch(const char *path, char *launch, size_t launch_size) {
    char m3u[MAX_PATH];
    const char *name;

    if (hasCue((char *)path, launch))
        return 1;
    name = strrchr(path, '/');
    if (!name || !name[1])
        return 0;
    ...
    if (dh != NULL) {
        ...
        while ((dp = readdir(dh)) != NULL) {
            if (hide(dp->d_name))
                continue;
            strcpy(tmp, dp->d_name);
            int type = dp->d_type == DT_DIR ? ENTRY_DIR : ENTRY_ROM;
            if (prefixMatch(COLLECTIONS_PATH, full_path) && dp->d_type != DT_DIR)
                type = ENTRY_DIR;
...
static Array *getEntries(char *path) {
    ...
    if (isConsoleDir(path)) { // top-level console folder, might collate
        ...
        // 1 because we want to keep the opening parenthesis to avoid collating "Game Boy Color" and "Game Boy Advance"
        // into "Game Boy" but conditional so we can continue to support a bare tag name as a folder name
        if (tmp)
            tmp[1] = '\0';
...
static void readyResumePath(char *rom_path, int type) {
    ...
    if (type == ENTRY_DIR && isVersionFolder(path)) {
        if (!readVersionChoice(path, path, sizeof(path)))
            return;
    ...
    char core_registry_name[256];
    getEmuName(path, core_registry_name);

    char rom_file[256];
    tmp = strrchr(path, '/') + 1;
    strcpy(rom_file, tmp);
    resolveSaveIdForPath(path, save_id, sizeof(save_id));

    snprintf(slot_path, sizeof(slot_path), "%s/%s/%s/selected-slot.state", SHARED_USERDATA_PATH, save_id, rom_file);
...
static int autoResume(void) {
    ...
    if (isVersionFolder(sd_path)) {
        ...
        if (isDirPath(sd_path)) {
            char cue_path[256];
            if (!resolveDirectoryLaunch(sd_path, cue_path, sizeof(cue_path)))
                return 0;
    ...
    char core_registry_path[256];
    getEmuPath(core_registry_name, core_registry_path);

    if (!exists(core_registry_path))
        return 0;

    char cmd[1024];
    if (bundle_path[0]) {
        sprintf(cmd, "MINUI_BUNDLE_PATH='%s' '%s/bin/minarch' '%s' '%s'", escapeSingleQuotes(bundle_path), SYSTEM_PATH,
                escapeSingleQuotes(core_registry_path), escapeSingleQuotes(sd_path));
    } else {
        sprintf(cmd, "'%s/bin/minarch' '%s' '%s'", SYSTEM_PATH, escapeSingleQuotes(core_registry_path),
...
static void openRom(char *path, char *last) {
    ...
    char save_id[256];
    getEmuName(sd_path, core_registry_name);
    resolveSaveIdForPath(sd_path, save_id, sizeof(save_id));
    ...
    if (should_resume) {
        ...
        if (has_m3u) {
            ...
            snprintf(disc_path_path, sizeof(disc_path_path), "%s/%s/%s/slot %d/disc.txt", SHARED_USERDATA_PATH, save_id,
                     rom_file, atoi(slot) + 1);
...
static void openDirectory(char *path, int auto_launch, int force_chooser) {
    ...
    if (isVersionFolder(path) && auto_launch && !force_chooser &&
        ...
        if (isDirPath(auto_path)) {
            char cue_path[256];
            if (resolveDirectoryLaunch(auto_path, cue_path, sizeof(cue_path))) {
...
static void Entry_open(Entry *self, int force_version_chooser) {
    ...
    if (top->version_chooser) {
        ...
        char cue_path[256];
        if (resolveDirectoryLaunch(self->path, cue_path, sizeof(cue_path))) {
    ...
    while (last->count > 0) {
        ...
        if (!exactMatch(path, ROMS_PATH)) { // romsDir is effectively root as far as restoring state after a game
            ...
            if (suffixMatch(")", path) && isConsoleDir(path)) {
                ...
                if (tmp)
                    tmp[1] = '\0'; // 1 because we want to keep the opening parenthesis to avoid collating "Game Boy
                                   // Color" and "Game Boy Advance" into "Game Boy"
            }
            ...
            for (int i = 0; i < top->entries->count; i++) {
                ...
                if (exactMatch(entry->path, path) ||
                    ...
                    if (last->count == 0 && !exactMatch(entry->path, FAUX_RECENT_PATH) &&
                        !(!exactMatch(entry->path, COLLECTIONS_PATH) && prefixMatch(COLLECTIONS_PATH, entry->path)))
...
int main(int argc, char *argv[]) {
    ...
    PAD_reset();
    int dirty = 1;
    enum LauncherScreen current_screen = SCREEN_BROWSER;
    ...
    while (!quit) {
        ...
        if (PAD_tappedMenu(now)) {
            if (current_screen == SCREEN_SETTINGS) {
                if (!SETTINGS_handleMenuToggle(&dirty)) {
                    current_screen = SCREEN_BROWSER;
                    SETTINGS_close();
                    if (!HAS_POWER_BUTTON && !simple_mode)
                        PWR_disableSleep();
                }
            ...
            if (total > 0 && can_resume && PAD_justReleased(BTN_RESUME)) {
                ...
                Entry_open(top->entries->items[top->selected], PAD_isPressed(BTN_SELECT));
        ...
        if (dirty) {
            ...
            int had_thumb = 0;
            if (current_screen == SCREEN_BROWSER && total > 0) {
                // simple thumbnail support a thumbnail for a file or folder
                // named NAME.EXT with a corresponding /.res/NAME.EXT.png
            ...
            if (current_screen == SCREEN_SETTINGS) {
                ...
                struct ui_list_view browser_view;
                struct ui_row browser_rows[MAIN_ROW_COUNT];

                UI_LIST_initView(&browser_view);
                browser_view.rows = browser_rows;
                browser_view.count = top->end - top->start;
                browser_view.selected = top->selected - top->start;
                browser_view.start = 0;
                browser_view.visible_rows = MAIN_ROW_COUNT;
                browser_view.header_width = had_thumb ? 0 : ow;
                browser_view.empty_message = "Empty folder";

                memset(browser_rows, 0, sizeof(browser_rows));
                if (total > 0) {
                    ...
                    for (int i = top->start, j = 0; i < top->end; i++, j++) {
                        ...
                        UI_LIST_initRow(&browser_rows[j], UI_ROW_KIND_BROWSER, entry_name,
                                        entry_unique ? entry_unique : "");
                        browser_rows[j].clip_right = (had_thumb && j != selected_row) ? ox : screen->w;
                    }
                }

                if (can_resume) {
                    browser_view.top_group.count = 1;
                    browser_view.top_group.primary = 0;
                    browser_view.top_group.align_right = 0;
                    browser_view.top_group.hints[0].button = UI_BUTTON_ID_X;
                    browser_view.top_group.hints[0].text = "RESUME";
                } else {
                    browser_view.top_group.count = 1;
                    browser_view.top_group.primary = 0;
                    browser_view.top_group.align_right = 0;
                    browser_view.top_group.hints[0].button =
                        BTN_SLEEP == BTN_POWER ? UI_BUTTON_ID_POWER : UI_BUTTON_ID_MENU;
                    browser_view.top_group.hints[0].label =
                        BTN_SLEEP == BTN_POWER || simple_mode ? UI_HINT_LABEL_NONE : UI_HINT_LABEL_SETTINGS;
                    browser_view.top_group.hints[0].text = BTN_SLEEP == BTN_POWER || simple_mode ? "SLEEP" : NULL;
                }

                if (total == 0) {
                    if (stack->count > 1) {
                        browser_view.bottom_group.count = 1;
                        browser_view.bottom_group.primary = 0;
                        browser_view.bottom_group.align_right = 1;
                        browser_view.bottom_group.hints[0].button = UI_BUTTON_ID_B;
                        browser_view.bottom_group.hints[0].label = UI_HINT_LABEL_BACK;
                    }
                } else if (stack->count > 1) {
                    browser_view.bottom_group.count = 2;
                    browser_view.bottom_group.primary = 1;
                    browser_view.bottom_group.align_right = 1;
                    browser_view.bottom_group.hints[0].button = UI_BUTTON_ID_B;
                    browser_view.bottom_group.hints[0].label = UI_HINT_LABEL_BACK;
                    browser_view.bottom_group.hints[1].button = UI_BUTTON_ID_A;
                    browser_view.bottom_group.hints[1].label = UI_HINT_LABEL_OPEN;
                } else {
                    browser_view.bottom_group.count = 1;
                    browser_view.bottom_group.primary = 0;
                    browser_view.bottom_group.align_right = 1;
                    browser_view.bottom_group.hints[0].button = UI_BUTTON_ID_A;
                    browser_view.bottom_group.hints[0].label = UI_HINT_LABEL_OPEN;
                }

                UI_LIST_drawRows(&browser_view, screen);
                UI_LIST_drawMessage(&browser_view, screen);
                UI_LIST_drawHints(&browser_view, screen, show_setting);
            }
