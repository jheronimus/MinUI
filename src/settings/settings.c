...
static void settings_syncPromptDialog(void) {
    ...
    if (prompt->type == SETTINGS_PROMPT_NONE) {
        if (settings.dialog.type == UI_DIALOG_PROGRESS || settings.dialog.type == UI_DIALOG_CONFIRM ||
    ...
    if (prompt->type == SETTINGS_PROMPT_BT_PROGRESS) {
        if (settings.dialog.type == UI_DIALOG_PROGRESS && !strcmp(settings.dialog.title, prompt->title) &&
            !strcmp(settings.dialog.detail, prompt->detail))
            return;
        UI_DIALOG_openProgress(&settings.dialog, prompt->title, prompt->message, prompt->detail, prompt->arg);
        return;
    }

    if (prompt->type == SETTINGS_PROMPT_BT_CONFIRM) {
        if (settings.dialog.type == UI_DIALOG_CONFIRM && !strcmp(settings.dialog.detail, prompt->detail) &&
            !strcmp(settings.dialog.arg, prompt->arg))
            return;
        UI_DIALOG_openConfirm(&settings.dialog, prompt->title, prompt->message, prompt->detail, prompt->arg);
        return;
    }

    if (prompt->type == SETTINGS_PROMPT_ERROR) {
        if (settings.dialog.type == UI_DIALOG_ERROR && !strcmp(settings.dialog.message, prompt->message) &&
            !strcmp(settings.dialog.detail, prompt->detail))
            return;
        UI_DIALOG_openError(&settings.dialog, prompt->title, prompt->message, prompt->detail);
    }
}
...
static int settings_handleDialogResult(int result) {
    ...
    case UI_DIALOG_WIFI_PASSPHRASE:
        if (result == UI_DIALOG_RESULT_CONFIRM) {
            if (!settings.dialog.allow_empty && !settings.dialog.keyboard.text[0]) {
                SETTINGS_setNotice(&settings, "Passphrase required");
                return 1;
            }
            rc = SETTINGS_JOBS_enqueueWifiConnect(settings.dialog.arg, settings.dialog.keyboard.text);
            if (rc != 0)
                SETTINGS_setNotice(&settings, "Wi-Fi connect queued failed");
        }
        settings_closeDialog();
        return 1;
    case UI_DIALOG_CONFIRM:
        if (result == UI_DIALOG_RESULT_CONFIRM) {
            rc = SETTINGS_JOBS_enqueue(SETTINGS_JOB_BT_DEVICE_CONFIRM, 0, settings.dialog.arg);
            if (rc != 0)
                SETTINGS_setNotice(&settings, "Bluetooth confirm queued failed");
        }
...
static void settings_clampSelection(void) {
    ...
    if (settings.selected[depth] >= settings.start[depth] + SETTINGS_ROW_COUNT)
        settings.start[depth] = settings.selected[depth] - SETTINGS_ROW_COUNT + 1;
...
static void settings_rebuild(void) {
    ...
    if (!settings_menuIsCustom(menu) && menu->build)
        settings.item_count = menu->build(&settings, &settings.snapshot, settings.items, SETTINGS_MAX_ITEMS);
...
static void settings_back(void) {
    if (settings.depth <= 0)
        return;
    settings_openMenu(settings.depth - 1, settings.menu_stack[settings.depth - 1], 0);
}

static int settings_hintIsSet(const struct ui_hint *hint) {
    return hint && ((hint->text && hint->text[0]) || hint->label != UI_HINT_LABEL_NONE);
}

static void settings_setHint(struct ui_hint *hint, int button, int label, const char *text) {
    if (!hint)
        return;
...
static void settings_primaryHint(const struct settings_item *item, struct ui_hint *hint) {
    if (!hint)
        return;
    memset(hint, 0, sizeof(*hint));
    if (!item)
...
static void settings_secondaryHint(const struct settings_item *item, struct ui_hint *hint) {
    if (!hint)
        return;
    memset(hint, 0, sizeof(*hint));
    if (!item)
...
static const char *settings_noticeMessage(void) {
    ...
    if ((menu_id == SETTINGS_MENU_WIFI || menu_id == SETTINGS_MENU_BT) &&
        (!settings.notice[0] || SDL_GetTicks() >= settings.notice_until))
        return "";
...
static void settings_buildHints(struct ui_list_view *view) {
    ...
    settings_setHint(&view->bottom_group.hints[0], UI_BUTTON_ID_B, UI_HINT_LABEL_BACK, NULL);
    if (settings_hintIsSet(&action_hint) && view->bottom_group.count < UI_LIST_MAX_HINTS) {
...
static void settings_drawCustomHints(SDL_Surface *screen, int show_setting) {
    ...
    if (!bottom.count) {
        ...
        settings_setHint(&bottom.hints[0], UI_BUTTON_ID_B, UI_HINT_LABEL_BACK, NULL);
    }
...
int SETTINGS_handleMenuToggle(int *dirty) {
    ...
    if (settings.dialog.type == UI_DIALOG_CONFIRM || settings.dialog.type == UI_DIALOG_ERROR)
        SETTINGS_JOBS_clearPrompt();
    settings_closeDialog();
    if (dirty)
        *dirty = 1;
...
void SETTINGS_update(int *dirty) {
    ...
    if (now != settings.last_clock_tick) {
        ...
        if (!settings_menuIsCustom(menu) && settings_currentMenuId() == SETTINGS_MENU_TIME) {
            settings_rebuild();
            if (dirty)
                *dirty = 1;
        }
...
void SETTINGS_handleInput(int *dirty, int *quit) {
    ...
    if (UI_DIALOG_isOpen(&settings.dialog)) {
        changed = settings_handleDialogResult(UI_DIALOG_handleInput(&settings.dialog));
        if (changed) {
            settings_rebuild();
            if (dirty)
                *dirty = 1;
    ...
    if (settings_menuIsCustom(menu)) {
        ...
        if (input_result == SETTINGS_CUSTOM_INPUT_BACK) {
            ...
            changed = 1;
        } else if (input_result == SETTINGS_CUSTOM_INPUT_NONE && PAD_justPressed(BTN_B)) {
            settings_back();
            changed = 1;
        }
        if (changed && dirty)
    ...
    if (PAD_justPressed(BTN_B)) {
        ...
        if (settings.item_count > 0)
            changed = SETTINGS_MENU_activateAux(&settings, &settings.items[settings.selected[settings.depth]]);
