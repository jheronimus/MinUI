...
///////////////////////////////////////
static void setNetworkBadge(struct settings_item *item, const struct settings_wifi_network *network) {
    const char *state;
    ...
    if (network->signal > 0) {
        snprintf(item->badge.text, sizeof(item->badge.text), "%s | %d%%", state, network->signal);
    } else {
...
static int wifi_networkIsConnected(const struct settings_screen *screen, const struct settings_item *item) {
    if (!screen || !item)
        return 0;
...
static void wifi_fillPrimaryHint(struct settings_screen *screen, const struct settings_item *item,
                                 struct ui_hint *hint) {
    ...
    case SETTINGS_ACTION_WIFI_TOGGLE:
        wifi_setHint(hint, UI_HINT_LABEL_NONE, item->data ? "DISABLE" : "ENABLE");
        return;
    case SETTINGS_ACTION_WIFI_CONNECT:
        ...
        if ((item->extra & SETTINGS_FLAG_WIFI_SECURE) && !(item->extra & SETTINGS_FLAG_WIFI_KNOWN)) {
...
static void wifi_fillAuxHint(const struct settings_item *item, struct ui_hint *hint) {
    if (!item || !hint || !item->enabled)
        return;
...
///////////////////////////////////////
static int hasNetworkRow(const struct settings_item *items, int count, const char *ssid) {
    int i;
...
static int addNetworks(struct settings_item *items, int max_items, int *count, const struct settings_snapshot *snapshot,
                       int group) {
    int i;
    ...
    for (i = 0; i < snapshot->wifi_network_count; i++) {
        ...
        if (*count >= max_items)
            return 0;
        item = &items[(*count)++];
        SETTINGS_initItem(item, SETTINGS_ITEM_ACTION, SETTINGS_ACTION_WIFI_CONNECT, network->ssid, "");
        item->data = WIFI_ROW_NETWORK;
        item->enabled = snapshot->wifi_enabled && !snapshot->wifi_busy;
        item->index = i;
...
int SETTINGS_WIFI_buildMenu(struct settings_screen *screen, const struct settings_snapshot *snapshot,
    ...
    if (snapshot->wifi_busy)
        toggle_label = snapshot->wifi_toggle_target ? "Enabling Wi-Fi" : "Disabling Wi-Fi";
    else
        toggle_label = snapshot->wifi_enabled ? "Disable Wi-Fi" : "Enable Wi-Fi";

    if (count >= max_items)
        return 0;
    item = &items[count++];
    SETTINGS_initItem(item, SETTINGS_ITEM_ACTION, SETTINGS_ACTION_WIFI_TOGGLE, toggle_label, "");
...
int SETTINGS_WIFI_activate(struct settings_screen *screen, const struct settings_item *item, int direction) {
    ...
    case SETTINGS_ACTION_WIFI_TOGGLE:
        ...
        rc = SETTINGS_JOBS_enqueue(SETTINGS_JOB_WIFI_TOGGLE, target, NULL);
        if (rc != 0)
    ...
    case SETTINGS_ACTION_WIFI_CONNECT:
        ...
        if (screen->snapshot.wifi_networks[item->index].connected) {
            rc = SETTINGS_JOBS_enqueue(SETTINGS_JOB_WIFI_DISCONNECT, 0, item->arg);
            if (rc != 0)
                SETTINGS_setNotice(screen, "Wi-Fi disconnect queued failed");
            return 1;
        }
        if ((item->extra & SETTINGS_FLAG_WIFI_SECURE) && !(item->extra & SETTINGS_FLAG_WIFI_KNOWN)) {
