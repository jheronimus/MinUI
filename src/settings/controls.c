...
static void controls_drawButton(const char *label, SDL_Surface *dst, int pressed, int x, int y, int width) {
    SDL_Rect point = {x, y};
    SDL_Surface *text;
    int len;
    ...
    GFX_blitPill(pressed ? ASSET_BUTTON : ASSET_HOLE, dst, &(SDL_Rect){point.x, point.y, width, SCALE1(BUTTON_SIZE)});
    SDL_BlitSurface(text, NULL, dst,
                    &(SDL_Rect){
...
void SETTINGS_CONTROLS_update(struct settings_screen *screen, const struct settings_snapshot *snapshot, int *dirty) {
    if (!screen)
        return;
    if (screen->controls.last_pressed != (uint32_t)pad.is_pressed || PAD_anyJustPressed() || PAD_anyJustReleased()) {
        screen->controls.last_pressed = pad.is_pressed;
        if (dirty)
            *dirty = 1;
    }
...
void SETTINGS_CONTROLS_draw(struct settings_screen *screen, SDL_Surface *dst) {
    ...
    has_l2 = controls_hasButton(BUTTON_L2, CODE_L2, JOY_L2) || AXIS_L2 != AXIS_NA;
    has_r2 = controls_hasButton(BUTTON_R2, CODE_R2, JOY_R2) || AXIS_R2 != AXIS_NA;
...

    {
        ...
        controls_drawButton("L1", dst, PAD_isPressed(BTN_L1), x + SCALE1(BUTTON_MARGIN), y + SCALE1(BUTTON_MARGIN), 0);
        if (has_l2)
            controls_drawButton("L2", dst, PAD_isPressed(BTN_L2), x + offset, y + SCALE1(BUTTON_MARGIN), 0);
    }

    {
        ...
        controls_drawButton(has_r2 ? "R2" : "R1", dst, PAD_isPressed(has_r2 ? BTN_R2 : BTN_R1),
                            x + SCALE1(BUTTON_MARGIN), y + SCALE1(BUTTON_MARGIN), 0);
        if (has_r2)
            controls_drawButton("R1", dst, PAD_isPressed(BTN_R1), x + offset, y + SCALE1(BUTTON_MARGIN), 0);
    }

    {
        ...
        SDL_FillRect(dst, &(SDL_Rect){x, y + SCALE1(PILL_SIZE / 2), SCALE1(PILL_SIZE), SCALE1(PILL_SIZE * 2)},
                     RGB_DARK_GRAY);
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, 0});
        controls_drawButton("U", dst, PAD_isPressed(BTN_DPAD_UP), x + o, y + o, 0);

        y += SCALE1(PILL_SIZE * 2);
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, 0});
        controls_drawButton("D", dst, PAD_isPressed(BTN_DPAD_DOWN), x + o, y + o, 0);

        x -= SCALE1(PILL_SIZE);
        y -= SCALE1(PILL_SIZE);
        SDL_FillRect(dst, &(SDL_Rect){x + SCALE1(PILL_SIZE / 2), y, SCALE1(PILL_SIZE * 2), SCALE1(PILL_SIZE)},
                     RGB_DARK_GRAY);
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, 0});
        controls_drawButton("L", dst, PAD_isPressed(BTN_DPAD_LEFT), x + o, y + o, 0);

        x += SCALE1(PILL_SIZE * 2);
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, 0});
        controls_drawButton("R", dst, PAD_isPressed(BTN_DPAD_RIGHT), x + o, y + o, 0);
    }

    {
        ...
        if (has_cz) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){bx, by, 0});
                controls_drawButton(labels[i], dst, PAD_isPressed(buttons[i]), bx + o, by + o, 0);
            }
        } else {
            x += SCALE1(PILL_SIZE);
            GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, 0});
    ...
    if (has_volume) {
        ...
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, SCALE1(98)});
        x += SCALE1(BUTTON_MARGIN);
        y += SCALE1(BUTTON_MARGIN);
        controls_drawButton("VOL. -", dst, PAD_isPressed(BTN_MINUS), x, y, width);
        x += width + SCALE1(BUTTON_MARGIN);
        controls_drawButton("VOL. +", dst, PAD_isPressed(BTN_PLUS), x, y, width);
    }

    if (has_power || has_menu) {
        int button_width = SCALE1(42);
        int pill_width = has_both ? SCALE1(42 * 2 + BUTTON_MARGIN * 3) : SCALE1(42 + BUTTON_MARGIN * 2);
        int x = (FIXED_WIDTH - pill_width) / 2;
        int y = oy + SCALE1(PILL_SIZE * 3);

        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, pill_width});
        x += SCALE1(BUTTON_MARGIN);
        y += SCALE1(BUTTON_MARGIN);
        if (has_menu) {
            controls_drawButton("MENU", dst, PAD_isPressed(BTN_MENU), x, y, button_width);
            x += button_width + SCALE1(BUTTON_MARGIN);
        }
        if (has_power)
            controls_drawButton("POWER", dst, PAD_isPressed(BTN_POWER), x, y, button_width);
    }

    {
        ...
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, meta_width});
        x += SCALE1(BUTTON_MARGIN);
        y += SCALE1(BUTTON_MARGIN);
        controls_drawButton("SELECT", dst, PAD_isPressed(BTN_SELECT), x, y, width);
        x += width + SCALE1(BUTTON_MARGIN);
        controls_drawButton("START", dst, PAD_isPressed(BTN_START), x, y, width);
    }
    ...
    if (has_l3) {
        ...
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, 0});
        controls_drawButton("L3", dst, PAD_isPressed(BTN_L3), x + o, y + o, 0);
    }
    ...
    if (has_r3) {
        ...
        GFX_blitPill(ASSET_DARK_GRAY_PILL, dst, &(SDL_Rect){x, y, 0});
        controls_drawButton("R3", dst, PAD_isPressed(BTN_R3), x + o, y + o, 0);
    }
}
