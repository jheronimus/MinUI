...
static void jobs_set_prompt_locked(int type, const char *title, const char *message, const char *detail,
                                   const char *arg) {
...
static void jobs_set_error_prompt(const char *title, const char *message, const char *detail) {
    pthread_mutex_lock(&jobs.lock);
    jobs_set_prompt_locked(SETTINGS_PROMPT_ERROR, title, message, detail, NULL);
...
static void jobs_wifi_pending_set(const char *ssid, const char *state) {
    if (!ssid || !ssid[0] || !state || !state[0])
        return;
    SETTINGS_copyText(jobs.wifi_pending_ssid, sizeof(jobs.wifi_pending_ssid), ssid);
    SETTINGS_copyText(jobs.wifi_pending_state, sizeof(jobs.wifi_pending_state), state);
...
static void jobs_bt_pending_set(const char *addr, const char *state) {
    if (!addr || !addr[0] || !state || !state[0])
        return;
    SETTINGS_copyText(jobs.bt_pending_addr, sizeof(jobs.bt_pending_addr), addr);
    SETTINGS_copyText(jobs.bt_pending_state, sizeof(jobs.bt_pending_state), state);
...
static const char *jobs_wifi_pending_for(const char *ssid) {
    ...
    if (!ssid || !ssid[0] || !jobs.wifi_pending_ssid[0] || strcmp(ssid, jobs.wifi_pending_ssid))
        return NULL;
    now = SDL_GetTicks();
    if (!jobs.wifi_pending_until || (int32_t)(now - jobs.wifi_pending_until) >= 0) {
...
static const char *jobs_bt_pending_for(const char *addr) {
    ...
    if (!addr || !addr[0] || !jobs.bt_pending_addr[0] || strcmp(addr, jobs.bt_pending_addr))
        return NULL;
    now = SDL_GetTicks();
    if (!jobs.bt_pending_until || (int32_t)(now - jobs.bt_pending_until) >= 0) {
...
static int jobs_wifi_should_clear_pending(const struct settings_wifi_network *net) {
    ...
    if (!strcmp(jobs.wifi_pending_state, "Connecting"))
        return net->connected && jobs_pending_min_elapsed(jobs.wifi_pending_started);
...
static int jobs_bt_should_clear_pending(const struct settings_bt_device *dev) {
    ...
    if (!strcmp(jobs.bt_pending_state, "Connecting"))
        return dev->connected && jobs_pending_min_elapsed(jobs.bt_pending_started);
    if (!strcmp(jobs.bt_pending_state, "Pairing"))
        return dev->paired && jobs_pending_min_elapsed(jobs.bt_pending_started);
    if (!strcmp(jobs.bt_pending_state, "Disconnecting"))
        return !dev->connected && jobs_pending_min_elapsed(jobs.bt_pending_started);
    return 0;
}

static int jobs_find_wifi_network(const struct settings_snapshot *snapshot, const char *ssid) {
    int i;
...
static int jobs_find_bt_device(const struct settings_snapshot *snapshot, const char *addr) {
    int i;
...
static void jobs_merge_cached_wifi(struct settings_snapshot *snapshot, const struct settings_snapshot *current) {
    int i;

    if (!snapshot || !current)
        return;
    for (i = 0; i < current->wifi_network_count && snapshot->wifi_network_count < SETTINGS_MAX_NETWORKS; i++) {
        if (jobs_find_wifi_network(snapshot, current->wifi_networks[i].ssid) >= 0)
            continue;
        snapshot->wifi_networks[snapshot->wifi_network_count++] = current->wifi_networks[i];
    }
}

static void jobs_merge_cached_bt(struct settings_snapshot *snapshot, const struct settings_snapshot *current) {
    int i;

    if (!snapshot || !current)
        return;
    for (i = 0; i < current->bt_device_count && snapshot->bt_device_count < SETTINGS_MAX_BT_DEVICES; i++) {
        if (jobs_find_bt_device(snapshot, current->bt_devices[i].addr) >= 0)
            continue;
        snapshot->bt_devices[snapshot->bt_device_count++] = current->bt_devices[i];
    }
}
...
static void jobs_publish_wifi_pending(const char *ssid, const char *state) {
    ...
    for (i = 0; i < jobs.snapshot.wifi_network_count; i++) {
        if (strcmp(jobs.snapshot.wifi_networks[i].ssid, ssid))
            continue;
        SETTINGS_copyText(jobs.snapshot.wifi_networks[i].state, sizeof(jobs.snapshot.wifi_networks[i].state), state);
        jobs.snapshot.generation++;
        break;
    }
    pthread_mutex_unlock(&jobs.lock);
...
static void jobs_publish_bt_pending(const char *addr, const char *state) {
    ...
    for (i = 0; i < jobs.snapshot.bt_device_count; i++) {
        if (strcmp(jobs.snapshot.bt_devices[i].addr, addr))
            continue;
        SETTINGS_copyText(jobs.snapshot.bt_devices[i].state, sizeof(jobs.snapshot.bt_devices[i].state), state);
        jobs.snapshot.generation++;
        break;
    }
    pthread_mutex_unlock(&jobs.lock);
...
static int jobs_ensure_state_dir(const char *path) {
    if (!path)
        return -EINVAL;
    if (!strncmp(path, SDCARD_PATH "/.minime/config/", strlen(SDCARD_PATH "/.minime/config/"))) {
...
static int jobs_read_timezone_offset(void) {
    ...
    if (jobs_read_state_number(TIMEZONE_STATE_FILE, "gmt_offset_minutes", &offset) != 0)
...
static int jobs_write_undervolt_level(int level) {
    ...
    while (line) {
        ...
        if (key[0] != '\0' && key[0] != '#' && (value = strchr(key, '=')) != NULL) {
            *value = '\0';
            if (!strcmp(key, "undervolt")) {
                pos += snprintf(out + pos, sizeof(out) - pos, "undervolt=%s\n", name);
                found = 1;
                line = strtok_r(NULL, "\n", &saveptr);
                continue;
            }
...
static void jobs_parse_release(struct settings_snapshot *snapshot) {
    ...
    SETTINGS_copyText(snapshot->model, sizeof(snapshot->model), PLAT_getModel());
}
...
static void jobs_fill_timezone(struct settings_snapshot *snapshot) {
    ...
    SETTINGS_TIMEZONE_format(offset_minutes, snapshot->timezone_text, sizeof(snapshot->timezone_text));
}
...
static void jobs_apply_pending(struct settings_snapshot *snapshot) {
    ...
    for (i = 0; i < snapshot->wifi_network_count; i++) {
        ...
        if (pending) {
            if (jobs_wifi_should_clear_pending(&snapshot->wifi_networks[i])) {
                jobs_wifi_pending_clear();
            } else {
                SETTINGS_copyText(snapshot->wifi_networks[i].state, sizeof(snapshot->wifi_networks[i].state), pending);
            }
        }
    }
    ...
    for (i = 0; i < snapshot->bt_device_count; i++) {
        ...
        if (pending) {
            if (jobs_bt_should_clear_pending(&snapshot->bt_devices[i])) {
                ...
                SETTINGS_copyText(snapshot->bt_devices[i].state, sizeof(snapshot->bt_devices[i].state), pending);
            }
        }
    }
}
...
static void jobs_refresh_snapshot(void) {
    ...
    next.ntp_running = jobs_pidfile_exists("/run/ntpd.pid") || jobs_pidfile_exists("/var/run/ntpd.pid");
    ...
    next.power_button_behavior = PWR_getPowerButtonBehavior();
    {
        ...
        next.power_undervolt_supported = traits && traits->undervolt_supported > 0;
    }
...
static void jobs_handle_wifi_toggle(const struct settings_job *job) {
    ...
    (void)jobs_write_state(WIFI_STATE_FILE, "wifi_enabled", job->value ? 1 : 0);
...
static void jobs_handle_wifi_connect(const struct settings_job *job) {
    ...
    pthread_mutex_lock(&jobs.lock);
    SETTINGS_copyText(connected, sizeof(connected), jobs.snapshot.wifi_connected_ssid);
    pthread_mutex_unlock(&jobs.lock);
    ...
    if (MINIME_wirelessWifiConnect(job->arg, job->secret[0] ? job->secret : NULL, 0) != 0) {
        jobs_wifi_pending_clear();
...
static void jobs_handle_bt_toggle(const struct settings_job *job) {
    ...
    if (MINIME_wirelessBluetoothSetEnabled(job->value) != 0) {
        jobs_set_error_prompt("Bluetooth", "Bluetooth change failed.", NULL);
        return;
    }
    (void)jobs_write_state(BT_STATE_FILE, "bluetooth_enabled", job->value ? 1 : 0);
    if (!job->value)
        jobs_bt_pending_clear();
...
static void jobs_handle_time_set(const struct settings_job *job) {
    if (PLAT_setDateTime(job->year, job->month, job->day, job->hour, job->minute, job->second) != 0) {
...
static void jobs_handle_timezone_set(const struct settings_job *job) {
    ...
    if (jobs_write_state(TIMEZONE_STATE_FILE, "gmt_offset_minutes", job->timezone_offset_minutes) != 0) {
        jobs_set_error_prompt("Time", "Timezone update failed.", NULL);
        return;
    }
    ...
    SETTINGS_TIMEZONE_format(job->timezone_offset_minutes, jobs.snapshot.timezone_text,
                             sizeof(jobs.snapshot.timezone_text));
    jobs.snapshot.generation++;
    pthread_mutex_unlock(&jobs.lock);
}

static void jobs_handle_power_sleep_timeout(const struct settings_job *job) {
    if (PWR_setSleepTimeoutMs(job->value) != 0 || jobs_write_power_policy() != 0)
        jobs_set_error_prompt("Power", "Sleep timeout update failed.", NULL);
}

static void jobs_handle_power_auto_shutdown_timeout(const struct settings_job *job) {
    if (PWR_setAutoShutdownTimeoutMs(job->value) != 0 || jobs_write_power_policy() != 0)
        jobs_set_error_prompt("Power", "Auto shutdown timeout update failed.", NULL);
}

static void jobs_handle_power_lid_behavior(const struct settings_job *job) {
    if (PWR_setLidBehavior(job->value) != 0 || jobs_write_power_policy() != 0)
        jobs_set_error_prompt("Power", "Lid behavior update failed.", NULL);
}

static void jobs_handle_power_button_behavior(const struct settings_job *job) {
    if (PWR_setPowerButtonBehavior(job->value) != 0 || jobs_write_power_policy() != 0)
        jobs_set_error_prompt("Power", "Power button behavior update failed.", NULL);
}

static void jobs_handle_power_undervolt(const struct settings_job *job) {
    if (jobs_write_undervolt_level(job->value) != 0)
        jobs_set_error_prompt("Power", "CPU undervolt update failed.", NULL);
}
...
static int jobs_sync_active_scanning(const struct settings_snapshot *snapshot) {
    ...
    if (jobs.active_menu == SETTINGS_MENU_WIFI && snapshot->wifi_enabled) {
        if (!snapshot->wifi_scanning && (!jobs.wifi_scan_after || (int32_t)(now - jobs.wifi_scan_after) >= 0)) {
            rc = MINIME_wirelessWifiSetScanning(1);
            jobs.wifi_scan_after = now + SETTINGS_WIFI_SCAN_MS;
            if (rc != 0)
                jobs_set_error_prompt("Wi-Fi", "Wi-Fi scan failed.", NULL);
            else
                return 1;
        }
    ...
    if (jobs.active_menu == SETTINGS_MENU_BT && snapshot->bt_enabled) {
        if (!snapshot->bt_scanning) {
            ...
            if (rc != 0)
                jobs_set_error_prompt("Bluetooth", "Bluetooth scan failed.", NULL);
            else
                return 1;
        }
        ...
        if (!jobs.bt_boot_scan_until) {
            ...
            if (rc != 0) {
                ...
                jobs.bt_boot_scan_until = now + SETTINGS_BT_BOOT_SCAN_MS;
                return 1;
            }
            ...
            if (snapshot->bt_scanning) {
                ...
                if (rc != 0)
                    jobs_set_error_prompt("Bluetooth", "Bluetooth stop scan failed.", NULL);
                else
                    return 1;
            }
        }
...
static void *settings_jobs_worker(void *arg) {
    ...
    while (jobs.running) {
        ...
        if (!has_job) {
            ...
            wait_ms = (jobs.active_menu == SETTINGS_MENU_WIFI || jobs.active_menu == SETTINGS_MENU_BT)
                          ? SETTINGS_POLL_MS
                          : 1000;
...
int SETTINGS_JOBS_enqueueTimeSet(int year, int month, int day, int hour, int minute, int second) {
    struct settings_job *job;
