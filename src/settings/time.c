...
static int time_selectedFieldCount(const struct settings_time_state *state) {
    if (!state)
        return 0;
    return state->show_24hour ? SETTINGS_TIME_FIELD_SECOND - SETTINGS_TIME_FIELD_YEAR + 1
...
static void time_syncTimezone(struct settings_screen *screen, const struct settings_snapshot *snapshot) {
    struct settings_time_state *state;
    ...
    state = &screen->time;
    state->timezone_index = SETTINGS_TIMEZONE_findIndex(snapshot->timezone_offset_minutes);
}

static void time_reset(struct settings_screen *screen, const struct settings_snapshot *snapshot) {
    struct settings_time_state *state;
...
static int time_ensureDigits(struct settings_screen *screen) {
    ...
    digits = SDL_CreateRGBSurface(SDL_SWSURFACE, SCALE2(120, 16), FIXED_DEPTH, RGBA_MASK_AUTO);
    if (!digits)
        return -ENOMEM;
    ...
    for (i = 0; chars[i]; i++) {
        ...
        SDL_BlitSurface(digit, NULL, digits,
                        &(SDL_Rect){
                            (i * SCALE1(DIGIT_WIDTH)) + (SCALE1(DIGIT_WIDTH) - digit->w) / 2,
...
static int time_blitDigit(struct settings_time_state *state, SDL_Surface *screen, int digit_index, int x, int y) {
    SDL_BlitSurface(state->digits, &(SDL_Rect){digit_index * SCALE1(DIGIT_WIDTH), 0, SCALE2(DIGIT_WIDTH, DIGIT_HEIGHT)},
                    screen, &(SDL_Rect){x, y});
    return x + SCALE1(DIGIT_WIDTH);
}

static int time_blitNumber(struct settings_time_state *state, SDL_Surface *screen, int value, int x, int y) {
    int digit;
...
static void time_formatSummary(const struct settings_time_state *state, char *dst, size_t dst_size) {
    int hour;
    ...
    if (state->show_24hour) {
        snprintf(dst, dst_size, "%04d/%02d/%02d %02d:%02d:%02d", state->year, state->month, state->day, state->hour,
    ...
    if (hour == 0)
        ...
        hour -= 12;
    snprintf(dst, dst_size, "%04d/%02d/%02d %02d:%02d:%02d %s", state->year, state->month, state->day, hour,
             state->minute, state->second, state->hour < 12 ? "AM" : "PM");
}
...
static void time_drawStatusRow(const char *label, const char *value, int y, int selected, SDL_Surface *screen) {
    ...
    GFX_truncateText(font.small, (char *)label, label_display, row_rect.w / 2, 0);
    ...
    value_rect.x = row_rect.x + row_rect.w - SCALE1(BUTTON_PADDING) - value_text->w;
...
static void time_drawClock(struct settings_screen *screen, SDL_Surface *dst) {
    ...
    if (!state->show_24hour) {
        x += SCALE1(10);
        ampm_text = TTF_RenderUTF8_Blended(font.large, state->hour < 12 ? "AM" : "PM", COLOR_WHITE);
        if (ampm_text) {
            ampm_width = ampm_text->w + SCALE1(2);
            SDL_BlitSurface(ampm_text, NULL, dst,
                            &(SDL_Rect){
                                x,
                                y - SCALE1(3),
                            });
...
static int time_adjustTimezone(struct settings_screen *screen, int delta) {
    ...
    rc = SETTINGS_JOBS_enqueueTimezoneSet(SETTINGS_TIMEZONE_offsetAt(state->timezone_index));
    if (rc != 0)
...
static int time_activateRow(struct settings_screen *screen) {
    ...
    case SETTINGS_TIME_ROW_CLOCK:
        ...
        return time_setClock(screen) ? SETTINGS_CUSTOM_INPUT_DIRTY : SETTINGS_CUSTOM_INPUT_NONE;
    default:
...
static int time_setClock(struct settings_screen *screen) {
    ...
    state = &screen->time;
    rc = SETTINGS_JOBS_enqueueTimeSet(state->year, state->month, state->day, state->hour, state->minute, state->second);
    if (rc != 0) {
...
///////////////////////////////////////
void SETTINGS_TIME_enter(struct settings_screen *screen, const struct settings_snapshot *snapshot) {
    time_reset(screen, snapshot);
}

void SETTINGS_TIME_update(struct settings_screen *screen, const struct settings_snapshot *snapshot, int *dirty) {
    struct settings_time_state *state;
    time_t now;
    ...
    if (state->refresh_after && (int32_t)(SDL_GetTicks() - state->refresh_after) >= 0) {
        time_syncTimezone(screen, snapshot);
        time_loadFromEpoch(screen, now);
        state->dirty = 0;
    ...
    if (state->dirty)
        return;
    if (state->timezone_index != SETTINGS_TIMEZONE_findIndex(snapshot->timezone_offset_minutes)) {
        time_syncTimezone(screen, snapshot);
        time_loadFromEpoch(screen, now);
        if (dirty)
            *dirty = 1;
...
int SETTINGS_TIME_handleInput(struct settings_screen *screen) {
    ...
    if (PAD_justRepeated(BTN_UP)) {
        if (state->editing_clock)
            return time_adjustField(screen, 1) ? SETTINGS_CUSTOM_INPUT_DIRTY : SETTINGS_CUSTOM_INPUT_NONE;
        return time_moveRow(state, -1) ? SETTINGS_CUSTOM_INPUT_DIRTY : SETTINGS_CUSTOM_INPUT_NONE;
    }
    ...
    if (PAD_justRepeated(BTN_LEFT)) {
        if (state->selected_row == SETTINGS_TIME_ROW_TIMEZONE)
            return time_adjustTimezone(screen, -1) ? SETTINGS_CUSTOM_INPUT_DIRTY : SETTINGS_CUSTOM_INPUT_NONE;
        if (state->selected_row == SETTINGS_TIME_ROW_CLOCK && state->editing_clock)
            return time_moveField(state, -1) ? SETTINGS_CUSTOM_INPUT_DIRTY : SETTINGS_CUSTOM_INPUT_NONE;
        return SETTINGS_CUSTOM_INPUT_NONE;
    }
    ...
    if (PAD_justPressed(BTN_SELECT))
        return time_set24Hour(state, !state->show_24hour) ? SETTINGS_CUSTOM_INPUT_DIRTY : SETTINGS_CUSTOM_INPUT_NONE;
    if (PAD_justPressed(BTN_X))
        return time_triggerSync(screen) ? SETTINGS_CUSTOM_INPUT_DIRTY : SETTINGS_CUSTOM_INPUT_NONE;
...
void SETTINGS_TIME_draw(struct settings_screen *screen, SDL_Surface *dst) {
    ...
    SETTINGS_TIMEZONE_format(SETTINGS_TIMEZONE_offsetAt(state->timezone_index), timezone_text, sizeof(timezone_text));

    y = SCALE1(PADDING);
    time_drawStatusRow("Current Time:", time_text, y, state->selected_row == SETTINGS_TIME_ROW_CURRENT_TIME, dst);
    y += SCALE1(PILL_SIZE);
    time_drawStatusRow("Current Timezone:", timezone_text, y, state->selected_row == SETTINGS_TIME_ROW_TIMEZONE, dst);
    y += SCALE1(PILL_SIZE);
    time_drawStatusRow("NTP:", time_ntpState(&screen->snapshot), y, state->selected_row == SETTINGS_TIME_ROW_NTP, dst);
    y += SCALE1(PILL_SIZE);
...
void SETTINGS_TIME_buildHints(struct settings_screen *screen, struct ui_hint_group *top, struct ui_hint_group *bottom) {
    struct settings_time_state *state;
    ...
    } else if (state->selected_row == SETTINGS_TIME_ROW_CLOCK && !time_editLocked(&screen->snapshot)) {
        bottom->count = 2;
        bottom->primary = 1;
        bottom->hints[1].button = UI_BUTTON_ID_A;
