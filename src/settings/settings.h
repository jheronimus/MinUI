...
#define SETTINGS_UNDERVOLT_OFF 0
#define SETTINGS_UNDERVOLT_L1 1
#define SETTINGS_UNDERVOLT_L2 2
#define SETTINGS_UNDERVOLT_L3 3
#define SETTINGS_UNDERVOLT_COUNT 4
...
enum settings_action_id {
    ...
    SETTINGS_ACTION_POWER_UNDERVOLT,
...
struct settings_snapshot {
    ...
    int power_undervolt_supported;
    int power_undervolt_level;
...
struct settings_menu {
    int id;
    const char *title;
    int (*build)(struct settings_screen *screen, const struct settings_snapshot *snapshot, struct settings_item *items,
                 int max_items);
    int (*activate)(struct settings_screen *screen, const struct settings_item *item, int direction);
    int (*activate_aux)(struct settings_screen *screen, const struct settings_item *item);
    void (*get_hint)(struct settings_screen *screen, const struct settings_item *item, struct ui_hint *hint);
    void (*get_aux_hint)(struct settings_screen *screen, const struct settings_item *item, struct ui_hint *hint);
    void (*enter)(struct settings_screen *screen, const struct settings_snapshot *snapshot);
    void (*leave)(struct settings_screen *screen);
    void (*update)(struct settings_screen *screen, const struct settings_snapshot *snapshot, int *dirty);
    int (*handle_input)(struct settings_screen *screen);
    void (*draw)(struct settings_screen *screen, SDL_Surface *surface);
    void (*build_hints)(struct settings_screen *screen, struct ui_hint_group *top, struct ui_hint_group *bottom);
};
...
///////////////////////////////////////
static inline void SETTINGS_copyText(char *dst, size_t size, const char *src) {
    if (!dst || !size)
        return;
    if (!src) {
        dst[0] = '\0';
...
static inline void SETTINGS_initItem(struct settings_item *item, int type, int action, const char *label,
                                     const char *desc) {
...
static inline void SETTINGS_setNotice(struct settings_screen *screen, const char *msg) {
    if (!screen)
        return;
