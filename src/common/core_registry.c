...
int CORE_REGISTRY_resolveLaunchForSystem(const char *system, char *out_core_id, size_t out_core_id_len,
    ...
    if (out_emu_path && out_emu_path_len) {
        snprintf(out_emu_path, out_emu_path_len, "%s/.cores/%s", sdcard, entry.filename);
...
int CORE_REGISTRY_resolveSaveIdForSystem(const char *system, char *out_save_id, size_t out_save_id_len) {
    CoreEntry entry;
...
int CORE_REGISTRY_resolveSaveIdForLaunchTag(const char *launch_tag, char *out_save_id, size_t out_save_id_len) {
    return CORE_REGISTRY_resolveSaveIdForSystem(launch_tag, out_save_id, out_save_id_len);
}
