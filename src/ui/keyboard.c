...
static const struct keyboard_layout keyboard_layout_lower = {
    .rows =
        {
            {keyboard_row0_lower, sizeof(keyboard_row0_lower) / sizeof(keyboard_row0_lower[0])},
            {keyboard_row1_lower, sizeof(keyboard_row1_lower) / sizeof(keyboard_row1_lower[0])},
            {keyboard_row2_lower, sizeof(keyboard_row2_lower) / sizeof(keyboard_row2_lower[0])},
            {keyboard_row3_lower, sizeof(keyboard_row3_lower) / sizeof(keyboard_row3_lower[0])},
            {keyboard_row4_controls, sizeof(keyboard_row4_controls) / sizeof(keyboard_row4_controls[0])},
        },
};
...
static const struct keyboard_layout keyboard_layout_upper = {
    .rows =
        {
            {keyboard_row0_upper, sizeof(keyboard_row0_upper) / sizeof(keyboard_row0_upper[0])},
            {keyboard_row1_upper, sizeof(keyboard_row1_upper) / sizeof(keyboard_row1_upper[0])},
            {keyboard_row2_upper, sizeof(keyboard_row2_upper) / sizeof(keyboard_row2_upper[0])},
            {keyboard_row3_upper, sizeof(keyboard_row3_upper) / sizeof(keyboard_row3_upper[0])},
            {keyboard_row4_controls, sizeof(keyboard_row4_controls) / sizeof(keyboard_row4_controls[0])},
        },
};

///////////////////////////////////////
static const struct keyboard_layout *keyboard_layoutFor(const struct ui_keyboard *keyboard) {
...
static void keyboard_drawCenteredText(TTF_Font *face, const char *text, SDL_Color color, SDL_Surface *screen, int x,
                                      int y, int w, int h) {
    SDL_Surface *surface;
...
static int keyboard_insert(struct ui_keyboard *keyboard, const char *text) {
    ...
    snprintf(keyboard->text + len, sizeof(keyboard->text) - len, "%s", text);
    return 1;
}
...
int UI_KEYBOARD_move(struct ui_keyboard *keyboard, int dx, int dy) {
    ...
    if (dx < 0) {
        keyboard->col = (keyboard->col - 1 + layout->rows[keyboard->row].count) % layout->rows[keyboard->row].count;
        return 1;
    }
    if (dx > 0) {
        keyboard->col = (keyboard->col + 1) % layout->rows[keyboard->row].count;
        return 1;
    }
    if (dy == 0)
        return 0;

    target_x = keyboard_keyCenterX(&layout->rows[keyboard->row], keyboard->col);
    keyboard->row = (keyboard->row + dy + UI_KEYBOARD_ROWS) % UI_KEYBOARD_ROWS;
    layout = keyboard_layoutFor(keyboard);
    keyboard->col = keyboard_colForX(&layout->rows[keyboard->row], target_x);
    return 1;
}
...
int UI_KEYBOARD_insertSelected(struct ui_keyboard *keyboard) {
    ...
    if (!strcmp(key, "SHIFT") || !strcmp(key, "DEL") || !strcmp(key, "DONE"))
        return 0;
...
const char *UI_KEYBOARD_getSelectedKey(const struct ui_keyboard *keyboard) {
    ...
    if (keyboard->row < 0 || keyboard->row >= UI_KEYBOARD_ROWS)
        return "";
    if (keyboard->col < 0 || keyboard->col >= layout->rows[keyboard->row].count)
        return "";
...
void UI_KEYBOARD_draw(const struct ui_keyboard *keyboard, SDL_Surface *screen, const SDL_Rect *rect) {
    ...
    for (row = 0; row < UI_KEYBOARD_ROWS; row++) {
        ...
        for (col = 0; col < row_def->count; col++) {
            ...
            int selected = (row == keyboard->row && col == keyboard->col);
            TTF_Font *face = strlen(key) > 1 ? font.tiny : font.small;

            GFX_blitPill(selected ? ASSET_WHITE_PILL : ASSET_DARK_GRAY_PILL, screen,
                         &(SDL_Rect){
                             x,
                             y,
                             width,
            ...
            keyboard_drawCenteredText(face, key, selected ? COLOR_BLACK : COLOR_WHITE, screen, x, y, width, row_height);
