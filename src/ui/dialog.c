...
///////////////////////////////////////
static void dialog_prepare(struct ui_dialog *dialog, int type, const char *title, const char *message,
    ...
    snprintf(dialog->message, sizeof(dialog->message), "%s", message ? message : "");
    snprintf(dialog->detail, sizeof(dialog->detail), "%s", detail ? detail : "");
...
static void dialog_drawHints(const struct ui_dialog *dialog, SDL_Surface *screen) {
...
static void dialog_drawText(const char *text, TTF_Font *font_ptr, SDL_Color color, SDL_Surface *screen, SDL_Rect *rect,
                            int leading) {
...
static void dialog_drawPromptText(SDL_Surface *screen, TTF_Font *face, const char *text, int x, int y,
                                  SDL_Color color) {
    SDL_Surface *surface;
...
static void dialog_drawButtons(const struct ui_dialog *dialog, SDL_Surface *screen, const SDL_Rect *panel) {
    SDL_Surface *text;
...
void UI_DIALOG_openWifiPassphrase(struct ui_dialog *dialog, const char *ssid) {
    dialog_prepare(dialog, UI_DIALOG_WIFI_PASSPHRASE, "Enter Wi-Fi password", "", ssid, ssid);
    dialog->allow_empty = 0;
}

void UI_DIALOG_openTextEntry(struct ui_dialog *dialog, const char *title, const char *detail, int allow_empty) {
    dialog_prepare(dialog, UI_DIALOG_TEXT_ENTRY, title, "", detail, "");
    dialog->allow_empty = allow_empty;
}

void UI_DIALOG_openProgress(struct ui_dialog *dialog, const char *title, const char *message, const char *detail,
                            const char *arg) {
    dialog_prepare(dialog, UI_DIALOG_PROGRESS, title, message, detail, arg);
}

void UI_DIALOG_openConfirm(struct ui_dialog *dialog, const char *title, const char *message, const char *detail,
                           const char *arg) {
    dialog_prepare(dialog, UI_DIALOG_CONFIRM, title, message, detail, arg);
    dialog->choice = 1;
}

void UI_DIALOG_openError(struct ui_dialog *dialog, const char *title, const char *message, const char *detail) {
...
int UI_DIALOG_handleInput(struct ui_dialog *dialog) {
    ...
    case UI_DIALOG_WIFI_PASSPHRASE:
        ...
        if (PAD_justRepeated(BTN_LEFT)) {
            ...
            const char *key = UI_KEYBOARD_getSelectedKey(&dialog->keyboard);

            if (!strcmp(key, "SHIFT"))
                return UI_KEYBOARD_toggleShift(&dialog->keyboard) ? UI_DIALOG_RESULT_UPDATE : UI_DIALOG_RESULT_NONE;
            else if (!strcmp(key, "DEL"))
                return UI_KEYBOARD_backspace(&dialog->keyboard) ? UI_DIALOG_RESULT_UPDATE : UI_DIALOG_RESULT_NONE;
            else if (!strcmp(key, "DONE")) {
                if (dialog->allow_empty || dialog->keyboard.text[0])
                    return UI_DIALOG_RESULT_CONFIRM;
            } else {
                return UI_KEYBOARD_insertSelected(&dialog->keyboard) ? UI_DIALOG_RESULT_UPDATE : UI_DIALOG_RESULT_NONE;
            }
    ...
    case UI_DIALOG_CONFIRM:
        if (PAD_tappedMenu(SDL_GetTicks()) || PAD_justPressed(BTN_B))
            return UI_DIALOG_RESULT_CANCEL;
        if (PAD_justRepeated(BTN_LEFT) || PAD_justRepeated(BTN_RIGHT) || PAD_justRepeated(BTN_UP) ||
            PAD_justRepeated(BTN_DOWN)) {
            dialog->choice = !dialog->choice;
            return UI_DIALOG_RESULT_UPDATE;
        }
        if (PAD_justPressed(BTN_A) || PAD_justPressed(BTN_START))
            return dialog->choice ? UI_DIALOG_RESULT_CONFIRM : UI_DIALOG_RESULT_CANCEL;
        return UI_DIALOG_RESULT_NONE;
    case UI_DIALOG_PROGRESS:
        if (PAD_tappedMenu(SDL_GetTicks()) || PAD_justPressed(BTN_B))
    ...
    case UI_DIALOG_ERROR:
    default:
        if (PAD_tappedMenu(SDL_GetTicks()) || PAD_justPressed(BTN_A) || PAD_justPressed(BTN_B) ||
...
void UI_DIALOG_draw(const struct ui_dialog *dialog, SDL_Surface *screen) {
    ...
    if (dialog->type == UI_DIALOG_TEXT_ENTRY || dialog->type == UI_DIALOG_WIFI_PASSPHRASE) {
        SDL_Rect input_rect;
        SDL_Rect keyboard_rect;
        int margin = SCALE1(PADDING);

        GFX_clear(screen);
        dialog_drawPromptText(screen, font.large, dialog->title, margin, SCALE1(PADDING), COLOR_WHITE);
        ...
        dialog_drawPromptText(screen, font.small, buffer, input_rect.x + SCALE1(6), input_rect.y + SCALE1(8),
                              COLOR_WHITE);

        if (dialog->detail[0]) {
            dialog_drawPromptText(screen, font.tiny, dialog->detail, margin, input_rect.y + input_rect.h + SCALE1(8),
                                  COLOR_LIGHT_TEXT);
        }
        ...
        keyboard_rect.h = screen->h - keyboard_rect.y - SCALE1(PADDING + PILL_SIZE + 18);
    ...
    dialog_drawText(dialog->title, font.large, COLOR_BLACK, screen, &rect, SCALE1(20));

    rect.y += SCALE1(PILL_SIZE + PADDING);
    rect.h = SCALE1(PILL_SIZE);
    dialog_drawText(dialog->message, font.small, COLOR_DARK_TEXT, screen, &rect, SCALE1(16));

    if (dialog->type != UI_DIALOG_TEXT_ENTRY && dialog->type != UI_DIALOG_WIFI_PASSPHRASE) {
        rect.y = panel.y + SCALE1(PILL_SIZE * 4);
        rect.h = panel.h - SCALE1(PILL_SIZE * 6);
        dialog_drawText(dialog->detail, font.small, COLOR_BLACK, screen, &rect, SCALE1(18));
