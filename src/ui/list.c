...
void UI_LIST_drawHintGroup(const struct ui_hint_group *group, SDL_Surface *screen) {
...
///////////////////////////////////////
static void ui_list_drawBrowserRow(const struct ui_row *row, const struct ui_list_view *view, SDL_Surface *screen,
                                   int row_index, int visible_index) {
    ...
    text_width = GFX_truncateText(font.large, subtitle ? subtitle : label, label_display, available_width,
                                  SCALE1(BUTTON_PADDING * 2));
    max_width = MIN(available_width, text_width);

    if (selected) {
        GFX_blitPill(ASSET_WHITE_PILL, screen, &(SDL_Rect){SCALE1(PADDING), row_y, max_width, SCALE1(PILL_SIZE)});
        text_color = COLOR_BLACK;
    } else if (subtitle) {
        GFX_truncateText(font.large, subtitle, subtitle_display, available_width, SCALE1(BUTTON_PADDING * 2));
        text = TTF_RenderUTF8_Blended(font.large, subtitle_display, COLOR_DARK_TEXT);
        if (text) {
            SDL_BlitSurface(text, &(SDL_Rect){0, 0, max_width - SCALE1(BUTTON_PADDING * 2), text->h}, screen,
                            &(SDL_Rect){SCALE1(PADDING + BUTTON_PADDING), row_y + SCALE1(4)});
            SDL_FreeSurface(text);
        }
        GFX_truncateText(font.large, label, label_display, available_width, SCALE1(BUTTON_PADDING * 2));
    }
...
static void ui_list_drawSettingsRow(const struct ui_row *row, const struct ui_list_view *view, SDL_Surface *screen,
                                    int row_index, int visible_index) {
...
void UI_LIST_initRow(struct ui_row *row, int kind, const char *label, const char *subtitle) {
    if (!row)
        return;
    ...
    snprintf(row->subtitle, sizeof(row->subtitle), "%s", subtitle ? subtitle : "");
}
...
void UI_LIST_drawRows(const struct ui_list_view *view, SDL_Surface *screen) {
    ...
    if (view->count <= 0) {
        if (view->empty_message)
            GFX_blitMessage(font.large, (char *)view->empty_message, screen, &(SDL_Rect){0, 0, screen->w, screen->h});
        return;
    }
...
void UI_LIST_drawMessage(const struct ui_list_view *view, SDL_Surface *screen) {
    ...
    GFX_blitText(font.tiny, (char *)view->message, SCALE1(12), COLOR_LIGHT_TEXT, screen, &rect);
}

void UI_LIST_drawHints(const struct ui_list_view *view, SDL_Surface *screen, int show_setting) {
    if (!view || !screen)
        return;
