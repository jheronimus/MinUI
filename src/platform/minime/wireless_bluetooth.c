...
static void bt_run_init(const char *script, const char *action) {
    ...
    snprintf(command, sizeof(command), "%s %s >/dev/null 2>&1", script, action);
    (void)system(command);
}
...
static DBusMessage *bt_call(DBusMessage *msg, int timeout_ms) {
    ...
    dbus_error_init(&err);
    reply = dbus_connection_send_with_reply_and_block(bt_state.conn, msg, timeout_ms, &err);
...
static int bt_variant_bool(DBusMessageIter *variant, int *value) {
    ...
    if (!variant || !value || dbus_message_iter_get_arg_type(variant) != DBUS_TYPE_VARIANT)
        return 0;
...
static int bt_variant_string(DBusMessageIter *variant, char *value, size_t value_size) {
    DBusMessageIter inner;
    const char *tmp;
    int type;
...
static int bt_variant_has_uuid(DBusMessageIter *variant, const char *uuid) {
    ...
    if (!variant || !uuid || dbus_message_iter_get_arg_type(variant) != DBUS_TYPE_VARIANT)
        return 0;
    dbus_message_iter_recurse(variant, &inner);
...
static int bt_device_kind(const char *icon, unsigned int class_id, DBusMessageIter *uuids) {
    ...
    if ((class_id & 0x1f00) == 0x0500)
        return SETTINGS_BT_DEVICE_GAMEPAD;
    if (uuids && bt_variant_has_uuid(uuids, "00001124-0000-1000-8000-00805f9b34fb"))
        return SETTINGS_BT_DEVICE_GAMEPAD;
    if (uuids && (bt_variant_has_uuid(uuids, "0000110b-0000-1000-8000-00805f9b34fb") ||
                  bt_variant_has_uuid(uuids, "0000110e-0000-1000-8000-00805f9b34fb") ||
                  bt_variant_has_uuid(uuids, "00001108-0000-1000-8000-00805f9b34fb")))
        return SETTINGS_BT_DEVICE_AUDIO;
...
static void bt_parse_adapter(DBusMessageIter *props, const char *path) {
    ...
    if (!traits || !name || strcmp(name + 1, traits->bluetooth_adapter))
        return;
    SETTINGS_copyText(bt_state.adapter_path, sizeof(bt_state.adapter_path), path);
...
static void bt_parse_device(DBusMessageIter *props, const char *path) {
    ...
    while (dbus_message_iter_get_arg_type(props) == DBUS_TYPE_DICT_ENTRY) {
        ...
        if (!strcmp(key, "Address")) {
            bt_variant_string(&value, device->addr, sizeof(device->addr));
        } else if (!strcmp(key, "Alias")) {
            bt_variant_string(&value, device->name, sizeof(device->name));
        } else if (!strcmp(key, "Name") && !device->name[0]) {
            bt_variant_string(&value, device->name, sizeof(device->name));
        } else if (!strcmp(key, "Icon")) {
            bt_variant_string(&value, device->icon, sizeof(device->icon));
    ...
    if (!device->addr[0])
        return;
    device->kind = bt_device_kind(device->icon, device->class_id, have_uuids ? &uuids : NULL);
...
static int bt_refresh_cache(void) {
    ...
    bt_reset_cache();
    reply = bt_call_noarg(BT_ROOT, BT_OBJMGR_IFACE, "GetManagedObjects", 4000);
    if (!reply)
        return -ENOTCONN;
...
static int bt_wait_for_adapter(void) {
    ...
    for (i = 0; i < BT_ADAPTER_WAIT_ATTEMPTS; i++) {
        if (bt_connect_bus() == 0 && bt_refresh_cache() == 0 && bt_state.adapter_path[0])
            return 0;
...
static int bt_set_bool_property(const char *path, const char *iface, const char *property, int enabled) {
    ...
    msg = dbus_message_new_method_call(BT_SERVICE, path, BT_PROPS_IFACE, "Set");
    if (!msg)
        return -ENOMEM;
...
int MINIME_wirelessBluetoothRefresh(struct settings_snapshot *snapshot) {
    ...
    for (i = 0; i < bt_state.device_count && out < SETTINGS_MAX_BT_DEVICES; i++) {
        ...
        SETTINGS_copyText(dst->name, sizeof(dst->name), src->name[0] ? src->name : src->addr);
...
int MINIME_wirelessBluetoothSetEnabled(int enabled) {
    ...
    if (enabled) {
        ...
        if (rc == 0)
            rc = bt_set_bool_property(bt_state.adapter_path, BT_ADAPTER_IFACE, "Powered", 1);
        if (rc != 0)
            bt_stop_services();
        return rc;
    }
    ...
    if (bt_connect_bus() == 0 && bt_refresh_cache() == 0 && bt_state.adapter_path[0]) {
        ...
        rc = bt_set_bool_property(bt_state.adapter_path, BT_ADAPTER_IFACE, "Powered", 0);
    }
    bt_stop_services();
    return rc;
}
...
int MINIME_wirelessBluetoothToggleDevice(const char *addr) {
    ...
    if (!device->paired) {
        if (bt_state.adapter_path[0])
            (void)bt_set_bool_property(bt_state.adapter_path, BT_ADAPTER_IFACE, "Pairable", 1);
        if (!bt_reply_ok(bt_call_noarg(device->path, BT_DEVICE_IFACE, "Pair", 30000)))
            return -EIO;
        (void)bt_set_bool_property(device->path, BT_DEVICE_IFACE, "Trusted", 1);
    }

    return bt_reply_ok(bt_call_noarg(device->path, BT_DEVICE_IFACE, "Connect", 12000)) ? 0 : -EIO;
}
