...
int main(int argc, char **argv) {
    ...
    if (SDL_Init(SDL_INIT_VIDEO) != 0)
        return 1;
    window = SDL_CreateWindow("", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, traits->screen_width,
                              traits->screen_height, SDL_WINDOW_SHOWN);
    ...
    if (traits->screen_rotation)
        SDL_RenderCopyEx(renderer, texture, NULL, &dst, traits->screen_rotation, NULL, SDL_FLIP_NONE);
    else
