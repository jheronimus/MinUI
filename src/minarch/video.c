...
void MSG_init(void) {
    digits =
        SDL_CreateRGBSurface(SDL_SWSURFACE, SCALE2(DIGIT_WIDTH * DIGIT_COUNT, DIGIT_HEIGHT), FIXED_DEPTH, 0, 0, 0, 0);
...
static int MSG_blitChar(int n, int x, int y) {
    if (n != DIGIT_SPACE) {
        SDL_BlitSurface(digits, &(SDL_Rect){n * SCALE1(DIGIT_WIDTH), 0, SCALE2(DIGIT_WIDTH, DIGIT_HEIGHT)}, screen,
                        &(SDL_Rect){x, y});
    }
    ...
    if (scaling == SCALE_NATIVE || scaling == SCALE_CROPPED) {
        ...
        if (scaling == SCALE_FULLSCREEN) {
            ...
            if (core_aspect > fixed_aspect) {
                ...
                double aspect_hr = ((double)aspect_h2) / DEVICE_HEIGHT;
                dst_w = scaled_w;
...
void video_refresh_callback_main(const void *data, unsigned width, unsigned height, size_t pitch) {
    ...
    if (renderer.dst_p == 0 || width != (unsigned)renderer.true_w || height != (unsigned)renderer.true_h) {
...
void video_refresh_callback(const void *data, unsigned width, unsigned height, size_t pitch) {
    ...
    if (thread_video) {
        ...
        if (backbuffer &&
            (backbuffer->w != (int)width || backbuffer->h != (int)height || backbuffer->pitch != (int)pitch)) {
