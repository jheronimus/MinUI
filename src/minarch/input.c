...
ButtonMapping default_button_mapping[] = {{"Up", RETRO_DEVICE_ID_JOYPAD_UP, BTN_ID_DPAD_UP},
...
ButtonMapping button_label_mapping[] = {{"NONE", -1, BTN_ID_NONE},
...
char *button_labels[] = {
    "NONE",    "UP",      "DOWN",    "LEFT",    "RIGHT",     "A",          "B",           "C",
    "X",       "Y",       "Z",       "START",   "SELECT",    "L1",         "R1",          "L2",
    "R2",      "L3",      "R3",      "MENU+UP", "MENU+DOWN", "MENU+LEFT",  "MENU+RIGHT",  "MENU+A",
    "MENU+B",  "MENU+C",  "MENU+X",  "MENU+Y",  "MENU+Z",    "MENU+START", "MENU+SELECT", "MENU+L1",
    "MENU+R1", "MENU+L2", "MENU+R2", "MENU+L3", "MENU+R3",   NULL,
};
...
void input_poll_callback(void) {
    ...
    for (int i = 0; i < SHORTCUT_COUNT; i++) {
        ...
        int btn = mapping->local == BTN_ID_NONE ? BTN_NONE : 1 << mapping->local;
        ...
        if (btn == BTN_NONE)
            continue;
        if (mapping->mod && PAD_isPressed(btn) && (mod_active || mod_released)) {
        ...
        if (mod_active || mod_released) {
            if (i == SHORTCUT_TOGGLE_FF) {
                ...
                if (PAD_justPressed(btn) || (!toggled_ff_on && shortcut_released)) {
                    if (PAD_isPressed(btn)) {
                        setRewindToggle(0);
                        setRewindPressed(0);
                    }
                    fast_forward = setFastForward(PAD_isPressed(btn) && mod_active);
                    if (mapping->mod)
                        ignore_menu = 1;
                }
    ...
    for (int i = 0; config.controls[i].name; i++) {
        ...
        int raw_btn = mapping->local == BTN_ID_NONE ? BTN_NONE : 1 << mapping->local;
        int btn = raw_btn;
