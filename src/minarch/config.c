...
static void Config_setRewindDefaults(void) {
    ...
    config.frontend.options[FE_OPT_REWIND_BUFFER].default_value = buffer_value;
    config.frontend.options[FE_OPT_REWIND_BUFFER].value = buffer_value;
    config.frontend.options[FE_OPT_REWIND_CAPTURE].default_value = capture_value;
    config.frontend.options[FE_OPT_REWIND_CAPTURE].value = capture_value;
    config.frontend.options[FE_OPT_REWIND_KEYFRAME].default_value = keyframe_value;
    config.frontend.options[FE_OPT_REWIND_KEYFRAME].value = keyframe_value;
    config.frontend.options[FE_OPT_REWIND_COMPRESSION].default_value = compression_value;
    config.frontend.options[FE_OPT_REWIND_COMPRESSION].value = compression_value;
...
void Config_syncFrontend(char *key, int value) {
    ...
    if (i >= FE_OPT_REWIND_ENABLE && i <= FE_OPT_REWIND_AUDIO && core.initialized) {
        Rewind_applyConfig();
    }
}
...
void Config_load(void) {
    ...
    snprintf(default_path, sizeof(default_path), "%s/default-%s.cfg", CORE_CONFIGS_PATH, tag_id);

    char device_default_path[MAX_PATH] = {0};
    if (config.device_tag) {
        snprintf(device_default_path, sizeof(device_default_path), "%s/default-%s-%s.cfg", CORE_CONFIGS_PATH, tag_id,
                 config.device_tag);
    }
