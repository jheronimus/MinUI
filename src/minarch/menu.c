...
void Menu_init(void) {
    ...
    if (ensure_dir_recursive(menu.minui_dir, 0755) != 0) {
        LOG_error("Error creating slot dir: %s (%s)\n", menu.minui_dir, strerror(errno));
    }
    snprintf(menu.slot_path, sizeof(menu.slot_path), "%s/selected-slot.state", menu.minui_dir);
...
void Menu_beforeSleep(void) {
    ...
    RTC_write();
    State_autosave();
    putFile(AUTO_RESUME_PATH, (game.bundle_path[0] ? game.bundle_path : game.path) + strlen(SDCARD_PATH));
...
static int OptionEmulator_optionChanged(MenuList *list, int i) {
    ...
    LOG_info("%s (%s) changed from `%s` (%s) to `%s` (%s)\n", item->name, item->key, item->values[option->value],
             option->values[option->value], item->values[item->value], option->values[item->value]);
...
static MenuList OptionControls_menu = {.type = MENU_INPUT,
...
static MenuList OptionShortcuts_menu = {.type = MENU_INPUT,
                                        .desc = "Press A to set and X to clear.\nSupports single button and "
                                                "MENU+button.",
...
static int Menu_options(MenuList *list) {
    ...
    int max_visible_options =
        (screen->h - ((SCALE1(PADDING + PILL_SIZE) * 2) + SCALE1(BUTTON_SIZE))) / SCALE1(BUTTON_SIZE);
            ...
            if (type == MENU_LIST) {
                ...
                for (int i = start, j = 0; i < end; i++, j++) {
                    ...
                    if (j == selected_row) {
                        ...
                        TTF_SizeUTF8(font.small, item->name, &w, NULL);
                        w += SCALE1(OPTION_PADDING * 2);
                        GFX_blitPill(ASSET_BUTTON, screen,
                    ...
                    SDL_BlitSurface(text, NULL, screen,
                                    &(SDL_Rect){ox + SCALE1(OPTION_PADDING), oy + SCALE1((j * BUTTON_SIZE) + 1)});
...
#define SCROLL_HEIGHT 4
                ...
                if (start > 0) {
                    GFX_blitAsset(ASSET_SCROLL_UP, NULL, screen, &(SDL_Rect){ox, SCALE1(PADDING) + oy});
                }
                if (end < count) {
                    GFX_blitAsset(ASSET_SCROLL_DOWN, NULL, screen,
                                  &(SDL_Rect){ox, screen->h - SCALE1(PADDING + PILL_SIZE + BUTTON_SIZE) + oy});
                }
            }
            ...
            if (desc) {
                ...
                GFX_blitText(font.tiny, desc, SCALE1(12), COLOR_WHITE, screen,
...
static void Menu_updateState(void) {
    ...
    {
        char slot_dir[256];
        snprintf(slot_dir, sizeof(slot_dir), "%s/slot %d", menu.minui_dir, menu.slot + 1);
        snprintf(menu.bmp_path, sizeof(menu.bmp_path), "%s/preview.bmp", slot_dir);
        snprintf(menu.txt_path, sizeof(menu.txt_path), "%s/disc.txt", slot_dir);
    }
...
void Menu_saveState(void) {
    ...
    if (!bitmap) {
        bitmap = SDL_CreateRGBSurfaceFrom(renderer.src, renderer.true_w, renderer.true_h, FIXED_DEPTH, renderer.src_p,
                                          RGBA_MASK_565);
    }
...
void Menu_loop(void) {
    menu.bitmap = SDL_CreateRGBSurfaceFrom(renderer.src, renderer.true_w, renderer.true_h, FIXED_DEPTH, renderer.src_p,
                                           RGBA_MASK_565);
    ...
    int dirty = 1;
    SDL_Surface *preview =
        SDL_CreateRGBSurface(SDL_SWSURFACE, DEVICE_WIDTH / 2, DEVICE_HEIGHT / 2, FIXED_DEPTH, RGBA_MASK_565);
    ...
    while (show_menu) {
        ...
        if (dirty) {
            ...
            int text_width =
                GFX_truncateText(font.large, rom_name, display_name, max_width, SCALE1(BUTTON_PADDING * 2));
            ...
            for (int i = 0; i < MENU_ITEM_COUNT; i++) {
                ...
                if (i == selected) {
                    if (menu.total_discs > 1 && i == ITEM_CONT) {
                        GFX_blitPill(ASSET_DARK_GRAY_PILL, screen,
                                     &(SDL_Rect){SCALE1(PADDING), SCALE1(oy + PADDING), screen->w - SCALE1(PADDING * 2),
                                                 SCALE1(PILL_SIZE)});
                    ...
                    GFX_blitPill(
                        ...
                        &(SDL_Rect){SCALE1(PADDING), SCALE1(oy + PADDING + (i * PILL_SIZE)), ow, SCALE1(PILL_SIZE)});
                    text_color = COLOR_BLACK;
                } else {
                ...
                SDL_BlitSurface(
                    text, NULL, screen,
                    &(SDL_Rect){SCALE1(PADDING + BUTTON_PADDING), SCALE1(oy + PADDING + (i * PILL_SIZE) + 4)});
                SDL_FreeSurface(text);
            }
