...
static void Core_resolveSaveId(char *out_save_id, size_t out_save_id_size) {
    if (!out_save_id || out_save_id_size == 0)
        return;
    out_save_id[0] = '\0';
    if (CORE_REGISTRY_resolveSaveIdForLaunchTag(core.tag, out_save_id, out_save_id_size) != 0) {
...
bool environment_callback(unsigned cmd, void *data) {
    ...
    case RETRO_ENVIRONMENT_GET_LOG_INTERFACE: {
        ...
        if (log_cb)
            log_cb->log = (void (*)(enum retro_log_level, const char *, ...))LOG_note;
        break;
    }
    ...
    case RETRO_ENVIRONMENT_GET_AUDIO_VIDEO_ENABLE: {
        enum retro_av_enable_flags *out_p = (enum retro_av_enable_flags *)data;
        if (out_p) {
            int out = 0;
    ...
    case RETRO_ENVIRONMENT_SET_DISK_CONTROL_EXT_INTERFACE: {
        const struct retro_disk_control_ext_callback *var = (const struct retro_disk_control_ext_callback *)data;
        if (var)
...
void Core_open(const char *core_path, const char *tag_name) {
    ...
    snprintf(core.config_dir, sizeof(core.config_dir), USERDATA_PATH "/%s-%s", core.tag, core.name);
    core.states_dir[0] = '\0';
    core.saves_dir[0] = '\0';
    {
        ...
        if (bios_dir && bios_dir[0]) {
            snprintf(core.bios_dir, sizeof(core.bios_dir), "%s", bios_dir);
        } else {
            snprintf(core.bios_dir, sizeof(core.bios_dir), "%s", SDCARD_PATH "/bios");
        }
    }
...
static void Core_refreshGameStoragePaths(void) {
    ...
    snprintf(core.states_dir, sizeof(core.states_dir), "%s/saves/%s/%s", storage_root, save_id, game.name);
    snprintf(core.saves_dir, sizeof(core.saves_dir), "%s/sram", core.states_dir);
    if (ensure_dir_recursive(core.config_dir, 0755) != 0) {
        LOG_error("Error creating core config dir: %s (%s)\n", core.config_dir, strerror(errno));
    }
...
int Core_load(void) {
    ...
    LOG_info("aspect_ratio: %f (%ix%i) fps: %f\n", a, av_info.geometry.base_width, av_info.geometry.base_height,
             core.fps);
